...
class FBEXPORT JObject : detail::JObjectBase {
 public:
  ...
  static constexpr const char *get_instantiated_java_descriptor() {
    return nullptr;
  }
  static constexpr const char *get_instantiated_base_name() {
    return nullptr;
  }
  ...
  /// Set the value of field. Any Java type is accepted, including the primitive
  /// types and raw reference types.
  template <typename T>
...
class FBEXPORT JClass : public JavaClass<JClass, JObject, jclass> {
 public:
  ...
  template <typename T>
  JField<enable_if_t<IsJniScalar<T>(), T>> getField(
      const char *name,
...
void registerNatives(
    const char *name,
    std::initializer_list<NativeMethod> methods);
...
/// Convenience functions to convert a std::string or const char* into a @ref
/// local_ref to a jstring
...
class FBEXPORT JPrimitiveArray : public JavaClass<
