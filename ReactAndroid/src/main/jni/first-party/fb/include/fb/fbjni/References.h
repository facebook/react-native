...
struct RefReprType<T *> {
  using type = typename std::conditional<
      HasJniRefRepr<T>::value,
      typename HasJniRefRepr<T>::type,
      JObjectWrapper<T *>>::type;
  static_assert(
      std::is_base_of<JObject, type>::value,
...
struct RefReprType<
    T,
    typename std::enable_if<std::is_base_of<JObject, T>::value, void>::type> {
  using type = T;
  static_assert(
      std::is_base_of<JObject, type>::value,
      "Repr type missing JObject base.");
...
/**
 * A non-owning variant of the smart references (a dumb reference). These
 * references still provide access to the functionality of the @ref
 * JObjectWrapper specializations including exception handling and ease of use.
 * Use this representation when you don't want to claim ownership of the
 * underlying reference (compare to using raw pointers instead of smart
 * pointers.) For symmetry use
