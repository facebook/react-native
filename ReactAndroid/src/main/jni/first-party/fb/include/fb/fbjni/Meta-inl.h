...
struct ArgsArraySetter<idx, Arg, Args...> {
  static void set(
      alias_ref<JArrayClass<jobject>::javaobject> array,
      Arg arg0,
      Args... args) {
...
inline bool needsSlowPath(alias_ref<jobject> obj) {
  ...
  // So, when we detect that case we must use the safe, slow workaround. That
  // is, we resolve the method id to the corresponding java.lang.reflect.Method
  // object and make the call via it's invoke() method.
...
#define DEFINE_PRIMITIVE_CALL(TYPE, METHOD)                           \
  ...
  inline TYPE JMethod<TYPE(Args...)>::operator()(                     \
      alias_ref<jobject> self, Args... args) {                        \
...
class JMethod<R(Args...)> : public JMethodBase {
 public:
  ...
  static_assert(
      IsPlainJniReference<JniRet>(),
      "JniRet must be a JNI reference");
...
#define DEFINE_PRIMITIVE_STATIC_CALL(TYPE, METHOD)                    \
  ...
  inline TYPE JStaticMethod<TYPE(Args...)>::operator()(               \
      alias_ref<jclass> cls, Args... args) {                          \
    const auto env = internal::getEnv();                              \
...
template <typename... Args>
inline void JNonvirtualMethod<void(Args...)>::
...
#define DEFINE_PRIMITIVE_NON_VIRTUAL_CALL(TYPE, METHOD)               \
  ...
  inline TYPE JNonvirtualMethod<TYPE(Args...)>::operator()(           \
      alias_ref<jobject> self, alias_ref<jclass> cls, Args... args) { \
    const auto env = internal::getEnv();                              \
...
inline void JField<T>::set(jobject object, T value) noexcept {
  internal::getEnv()->SetObjectField(
