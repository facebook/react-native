...
enable_if_t<IsPlainJniReference<T>(), local_ref<T>> static_ref_cast(
...
template <typename T, typename U>
enable_if_t<IsPlainJniReference<T>(), global_ref<T>> static_ref_cast(
...
template <typename T, typename U>
enable_if_t<IsPlainJniReference<T>(), alias_ref<T>> static_ref_cast(
...
auto dynamic_ref_cast(const RefType &ref) -> enable_if_t<
  ...
  static alias_ref<jclass> target_class =
      findClassStatic(jtype_traits<T>::base_name().c_str());
  if (!target_class) {
