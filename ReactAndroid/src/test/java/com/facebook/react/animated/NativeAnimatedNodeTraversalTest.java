...
public class NativeAnimatedNodeTraversalTest {
   ...
   *
   * <p>
   * Nodes are connected as follows (nodes IDs in parens): ValueNode(1) ->
   * StyleNode(2) -> PropNode(3)
   */
  private void createSimpleAnimatedViewWithOpacity(int viewTag, double opacity) {
    mNativeAnimatedNodesManager.createAnimatedNode(1, JavaOnlyMap.of("type", "value", "value", opacity, "offset", 0d));
  ...
  public void testFramesAnimation() {
    ...
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 1d), animationCallback);
  ...
  public void testFramesAnimationLoopsFiveTimes() {
    ...
    Callback animationCallback = mock(Callback.class);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 1d, "iterations", 5), animationCallback);
  ...
  public void testNodeValueListenerIfNotListening() {
    ...
    mNativeAnimatedNodesManager.startAnimatingNode(1, nodeId,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 1d), animationCallback);
  ...
  public void performSpringAnimationTestWithConfig(JavaOnlyMap config, boolean testForCriticallyDamped) {
    ...
    for (int i = 0; i < 3 * 60; i++) {
      reset(mUIManagerMock);
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
      verify(mUIManagerMock, atMost(1)).synchronouslyUpdateViewOnUIThread(eq(1000), stylesCaptor.capture());
    ...
    // verify that value has reached some maximum value that is greater than the
    // final value
    // (bounce)
  ...
  public void testUnderdampedSpringAnimation() {
    performSpringAnimationTestWithConfig(JavaOnlyMap.of("type", "spring", "stiffness", 230.2d, "damping", 22d, "mass",
        1d, "initialVelocity", 0d, "toValue", 1d, "restSpeedThreshold", 0.001d, "restDisplacementThreshold", 0.001d,
        "overshootClamping", false), false);
  }

  @Test
  public void testCriticallyDampedSpringAnimation() {
    performSpringAnimationTestWithConfig(JavaOnlyMap.of("type", "spring", "stiffness", 1000d, "damping", 500d, "mass",
        3.0d, "initialVelocity", 0d, "toValue", 1d, "restSpeedThreshold", 0.001d, "restDisplacementThreshold", 0.001d,
        "overshootClamping", false), true);
  }
  ...
  public void testSpringAnimationLoopsFiveTimes() {
    ...
    Callback animationCallback = mock(Callback.class);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "spring", "stiffness", 230.2d, "damping", 22d, "mass", 1d, "initialVelocity", 0d,
            "toValue", 1d, "restSpeedThreshold", 0.001d, "restDisplacementThreshold", 0.001d, "overshootClamping",
            false, "iterations", 5),
        animationCallback);
    ...
    for (int i = 0; i < 3 * 60 * 5; i++) {
      ...
      if (didComeToRest && currentValue == 0d && Math.abs(Math.abs(currentValue - previousValue) - 1d) < 0.001d) {
        numberOfResets++;
      }

      // verify that an animation step is relatively small, unless it has come to rest
      // and reset
      if (!didComeToRest)
        assertThat(Math.abs(currentValue - previousValue)).isLessThan(0.12d);

      // record that the animation did come to rest when it rests on toValue
      didComeToRest = Math.abs(currentValue - 1d) < 0.001d && Math.abs(currentValue - previousValue) < 0.001d;
      previousValue = currentValue;
    }
    // verify that we've reach the final value at the end of animation
    assertThat(previousValue).isEqualTo(1d);
  ...
  public void testDecayAnimation() {
    ...
    Callback animationCallback = mock(Callback.class);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "decay", "velocity", 0.5d, "deceleration", 0.998d), animationCallback);
  ...
  public void testDecayAnimationLoopsFiveTimes() {
    ...
    Callback animationCallback = mock(Callback.class);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "decay", "velocity", 0.5d, "deceleration", 0.998d, "iterations", 5), animationCallback);
    ...
    for (int i = 0; i < 3 * 60 * 5; i++) {
      ...
      // Test if animation has come to rest using the 0.1 threshold from
      // DecayAnimation.java
  ...
  /**
   * Creates a following graph of nodes: Value(1, firstValue) ----> Add(3) --->
   * Style(4) ---> Props(5) ---> View(viewTag) | Value(2, secondValue) --+
   *
   * <p>
   * Add(3) node maps to a "translateX" attribute of the Style(4) node.
   */
  private void createAnimatedGraphWithAdditionNode(int viewTag, double firstValue, double secondValue) {
    mNativeAnimatedNodesManager.createAnimatedNode(1,
    ...
    mNativeAnimatedNodesManager.createAnimatedNode(4,
        JavaOnlyMap.of("type", "style", "style", JavaOnlyMap.of("translateX", 3)));
    mNativeAnimatedNodesManager.createAnimatedNode(5,
  ...
  public void testAdditionNode() {
    ...
    JavaOnlyArray frames = JavaOnlyArray.of(0d, 1d);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 101d), animationCallback);

    mNativeAnimatedNodesManager.startAnimatingNode(2, 2,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 1010d), animationCallback);
  ...
  /**
   * Verifies that {@link NativeAnimatedNodesManager#runUpdates} updates the view
   * correctly in case when one of the addition input nodes has started animating
   * while the other one has not.
   *
   * <p>
   * We expect that the output of the addition node will take the starting value
   * of the second input node even though the node hasn't been connected to an
   * active animation driver.
   */
  @Test
  ...
  /**
   * Verifies that {@link NativeAnimatedNodesManager#runUpdates} updates the view
   * correctly in case when one of the addition input nodes animation finishes
   * before the other.
   *
   * <p>
   * We expect that the output of the addition node after one of the animation has
   * finished will take the last value of the animated node and the view will
   * receive updates up until the second animation is over.
   */
  @Test
  public void testViewReceiveUpdatesWhenOneOfAnimationHasFinished() {
    ...
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", firstFrames, "toValue", 200d), animationCallback);
    ...
    mNativeAnimatedNodesManager.startAnimatingNode(2, 2,
        JavaOnlyMap.of("type", "frames", "frames", secondFrames, "toValue", 1010d), animationCallback);
    ...
    for (int i = 1; i < secondFrames.size(); i++) {
      ...
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
      verify(mUIManagerMock).synchronouslyUpdateViewOnUIThread(eq(50), stylesCaptor.capture());
      assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(1200d + secondFrames.getDouble(i) * 10d);
    }
  ...
  public void testMultiplicationNode() {
    ...
    mNativeAnimatedNodesManager.createAnimatedNode(2, JavaOnlyMap.of("type", "value", "value", 5d, "offset", 0d));
    ...
    JavaOnlyArray frames = JavaOnlyArray.of(0d, 1d);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 2d), animationCallback);

    mNativeAnimatedNodesManager.startAnimatingNode(2, 2,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 10d), animationCallback);
  ...
  /**
   * This test verifies that when {@link NativeAnimatedModule#stopAnimation} is
   * called the animation will no longer be updating the nodes it has been
   * previously attached to and that the animation callback will be triggered with
   * {@code {finished: false}}
   */
  @Test
  public void testHandleStoppingAnimation() {
    ...
    Callback animationCallback = mock(Callback.class);
    mNativeAnimatedNodesManager.startAnimatingNode(404, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 1d), animationCallback);
    ...
    mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
    mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
    verify(mUIManagerMock, times(2)).synchronouslyUpdateViewOnUIThread(anyInt(), any(ReadableMap.class));
    verifyNoMoreInteractions(animationCallback);
    ...
    reset(animationCallback);
    reset(mUIManagerMock);
    // Run "update" loop a few more times -> we expect no further updates nor
    // callback calls to be
    // triggered
    for (int i = 0; i < 5; i++) {
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
    }
  ...
  public void testInterpolationNode() {
    mNativeAnimatedNodesManager.createAnimatedNode(1, JavaOnlyMap.of("type", "value", "value", 10d, "offset", 0d));

    mNativeAnimatedNodesManager.createAnimatedNode(2,
        JavaOnlyMap.of("type", "interpolation", "inputRange", JavaOnlyArray.of(10d, 20d), "outputRange",
            JavaOnlyArray.of(0d, 1d), "extrapolateLeft", "extend", "extrapolateRight", "extend"));
    ...
    JavaOnlyArray frames = JavaOnlyArray.of(0d, 0.2d, 0.4d, 0.6d, 0.8d, 1d);
    mNativeAnimatedNodesManager.startAnimatingNode(1, 1,
        JavaOnlyMap.of("type", "frames", "frames", frames, "toValue", 20d), animationCallback);
  ...
  public void testNativeAnimatedEventDoUpdate() {
    ...
    mNativeAnimatedNodesManager.addAnimatedEventToView(viewTag, "topScroll",
        JavaOnlyMap.of("animatedValueTag", 1, "nativeEventPath", JavaOnlyArray.of("contentOffset", "y")));
  ...
  public void testNativeAnimatedEventDoNotUpdate() {
    ...
    mNativeAnimatedNodesManager.addAnimatedEventToView(viewTag, "otherEvent",
        JavaOnlyMap.of("animatedValueTag", 1, "nativeEventPath", JavaOnlyArray.of("contentOffset", "y")));

    mNativeAnimatedNodesManager.addAnimatedEventToView(999, "topScroll",
        JavaOnlyMap.of("animatedValueTag", 1, "nativeEventPath", JavaOnlyArray.of("contentOffset", "y")));
  ...
  public void testNativeAnimatedEventCustomMapping() {
    ...
    PowerMockito.when(mUIManagerMock.getConstants()).thenAnswer(new Answer<Object>() {
      @Override
      public Object answer(InvocationOnMock invocation) throws Throwable {
        return MapBuilder.of("customDirectEventTypes",
    ...
    mNativeAnimatedNodesManager.addAnimatedEventToView(viewTag, "onScroll",
        JavaOnlyMap.of("animatedValueTag", 1, "nativeEventPath", JavaOnlyArray.of("contentOffset", "y")));
  ...
  /**
   * Creates a following graph of nodes: Value(3, initialValue) ----> Style(4)
   * ---> Props(5) ---> View(viewTag)
   *
   * <p>
   * Value(3) is set to track Value(1) via Tracking(2) node with the provided
   * animation config
   */
  private void createAnimatedGraphWithTrackingNode(int viewTag, double initialValue, JavaOnlyMap animationConfig) {
    mNativeAnimatedNodesManager.createAnimatedNode(1,
        JavaOnlyMap.of("type", "value", "value", initialValue, "offset", 0d));
    mNativeAnimatedNodesManager.createAnimatedNode(3,
        JavaOnlyMap.of("type", "value", "value", initialValue, "offset", 0d));

    mNativeAnimatedNodesManager.createAnimatedNode(2, JavaOnlyMap.of("type", "tracking", "animationId", 70, "value", 3,
        "toValue", 1, "animationConfig", animationConfig));
  ...
  /**
   * In this test we verify that when value is being tracked we can update
   * destination value in the middle of ongoing animation and the animation will
   * update and animate to the new spot. This is tested using simple 5 frame
   * backed timing animation.
   */
  @Test
  public void testTracking() {
    ...
    // update "toValue" to 100, we expect tracking animation to animate now from 0
    // to 100 in 5 steps
    ...
    for (int i = 0; i < frames.size(); i++) {
      ...
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
      verify(mUIManagerMock).synchronouslyUpdateViewOnUIThread(eq(1000), stylesCaptor.capture());
      assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(frames.getDouble(i) * 100d);
    }
    ...
    for (int i = 0; i < 2; i++) {
      ...
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
      verify(mUIManagerMock).synchronouslyUpdateViewOnUIThread(eq(1000), stylesCaptor.capture());
      assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(100d * (1d - frames.getDouble(i)));
    }

    // at this point we expect tracking value to be at 75
    assertThat(((ValueAnimatedNode) mNativeAnimatedNodesManager.getNodeById(3)).getValue()).isEqualTo(75d);

    // we update "toValue" again to 100 and expect the animation to restart from the
    // current place
    mNativeAnimatedNodesManager.setAnimatedNodeValue(1, 100d);
    mNativeAnimatedNodesManager.runUpdates(nextFrameTime()); // kick off the animation
    ...
    for (int i = 0; i < frames.size(); i++) {
      ...
      mNativeAnimatedNodesManager.runUpdates(nextFrameTime());
      verify(mUIManagerMock).synchronouslyUpdateViewOnUIThread(eq(1000), stylesCaptor.capture());
      assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(50d + 50d * frames.getDouble(i));
    }
  }

  /**
   * In this test we verify that when tracking is set up for a given animated node
   * and when the animation settles it will not be registered as an active
   * animation and therefore will not consume resources on running the animation
   * that has already completed. Then we verify that when the value updates the
   * animation will resume as expected and the complete again when reaches the
   * end.
   */
  @Test
  public void testTrackingPausesWhenEndValueIsReached() {
    ...
    verify(mUIManagerMock, times(frames.size())).synchronouslyUpdateViewOnUIThread(eq(1000), any(ReadableMap.class));
  ...
  /**
   * In this test we verify that when tracking is configured to use spring
   * animation and when the destination value updates the current speed of the
   * animated value will be taken into account while updating the spring animation
   * and it will smoothly transition to the new end value.
   */
  @Test
  public void testSpringTrackingRetainsSpeed() {
    // this spring config corresponds to tension 20 and friction 0.5 which makes the
    // spring settle
    // very slowly
    JavaOnlyMap springConfig = JavaOnlyMap.of("type", "spring", "restSpeedThreshold", 0.001, "mass", 1d,
        "restDisplacementThreshold", 0.001, "initialVelocity", 0.5d, "damping", 2.5, "stiffness", 157.8,
        "overshootClamping", false);

    createAnimatedGraphWithTrackingNode(1000, 0d, springConfig);

    // update "toValue" to 1, we expect tracking animation to animate now from 0 to
    // 1
    mNativeAnimatedNodesManager.setAnimatedNodeValue(1, 1d);
    mNativeAnimatedNodesManager.runUpdates(nextFrameTime());

    // we run several steps of animation until the value starts bouncing, has
    // negative speed and
    ...
    // we now update "toValue" to 1.5 but since the value have negative speed and
    // has also pretty
    // low friction we expect it to keep going in the opposite direction for a few
    // more frames
