...
NSString *const RCTAppearanceColorSchemeNoPreference = @"no-preference";
  ...
  if (@available(iOS 13.0, *)) {
    ...
    dispatch_once(&onceToken, ^{
      appearances = @{
                      ...
                      @(UIUserInterfaceStyleUnspecified): RCTAppearanceColorSchemeNoPreference
                      };
    });

    traitCollection = traitCollection ?: [UITraitCollection currentTraitCollection];
    return appearances[@(traitCollection.userInterfaceStyle)] ?: RCTAppearanceColorSchemeNoPreference;
  }
#endif

  return RCTAppearanceColorSchemeNoPreference;
}
...
{
  return @{ @"colorScheme": RCTColorSchemePreference(nil)};
}
