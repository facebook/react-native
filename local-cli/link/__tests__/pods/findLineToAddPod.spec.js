...
describe('pods::findLineToAddPod', () => {
  ...
  it('returns correct line number for Simple Podfile', () => {
    ...
    expect(findLineToAddPod(podfile, LINE_AFTER_TARGET_IN_TEST_PODFILE)).toBe(7);
  });
  ...
  it('returns correct line number for Podfile with target', () => {
    ...
    expect(findLineToAddPod(podfile, LINE_AFTER_TARGET_IN_TEST_PODFILE)).toBe(21);
  });
  ...
  it('returns correct line number for Podfile with function', () => {
    ...
    expect(findLineToAddPod(podfile, LINE_AFTER_TARGET_IN_TEST_PODFILE)).toBe(26);
  });
});
