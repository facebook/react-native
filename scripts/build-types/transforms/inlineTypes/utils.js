...
import {
  MAX_ALIAS_REFERENCES_FOR_INLINING,
  aliasInlineDespiteReferenceLimit,
  aliasInliningBlocklist,
  aliasInliningBlocklistRecursive,
} from './config';
import {
  insideArrayLayer,
  insideExtendsLayer,
...
export function shouldSkipInliningRecursively(
  ...
  return aliasInliningBlocklistRecursive.has(alias);
}
...
export function shouldSkipInliningType(
  ...
  // Explicit blocklist
  if (aliasInliningBlocklist.has(alias)) {
    return true;
  }
  ...
  if (
    referenceCount >= MAX_ALIAS_REFERENCES_FOR_INLINING &&
    !aliasInlineDespiteReferenceLimit.has(alias)
  ) {
    return true;
  }
