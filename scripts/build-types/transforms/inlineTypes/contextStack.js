...
export function pushLayer(state: VisitorState, layer: StackLayer) {
  state.stack.push(layer);
}

export function popLayer(state: VisitorState, type: StackLayer['type']) {
...
export function isDefiningType(state: VisitorState, alias: string): boolean {
  return state.parentTypeAliases?.has(alias) ?? false;
}

export function insideKeyofLayer(state: VisitorState): boolean {
  return state.stack.some(layer => layer.type === 'keyof');
}

export function insideOmitLayer(state: VisitorState): boolean {
  return state.stack.some(layer => layer.type === 'omit');
}

export function insideUnionLayer(state: VisitorState): boolean {
  return state.stack.some(layer => layer.type === 'union');
}

export function insideExtendsLayer(state: VisitorState): boolean {
  return state.stack.some(layer => layer.type === 'extends');
}

export function insideArrayLayer(state: VisitorState): boolean {
...
export function insideIndexedAccessLayer(state: VisitorState): boolean {
  return state.stack.some(layer => layer.type === 'indexedAccess');
}

export function insideNoInlineLayer(state: VisitorState): boolean {
