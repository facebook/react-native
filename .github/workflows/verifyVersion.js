...
module.exports = async (github, context) => {
    ...
    const issueVersion = parseVersionFromString(getReactNativeVersionIfExists(issue));
...
function isVersionSupported(actualVersion, latestVersion, releaseNames) {
    ...
    if (actualVersion.major != latestVersion.major ||
    ...
    const latestPatchForVersion = getLatestPatchForVersion(actualVersion, releaseNames);
...
function parseVersionFromString(version) {
    ...
    const semverRegex = /(?<major>[0-9]{1,2})\.(?<minor>[0-9]{1,2})\.(?<patch>[0-9]{1,2})(-[rR]{1}[cC]{1}\.(?<prerelease>[0-9]{1,2}))?/;
    const versionMatch = version.match(semverRegex);
    const { major, minor, patch, prerelease } = versionMatch.groups;
    return { major: parseInt(major), minor: parseInt(minor), patch: parseInt(patch), prerelease: parseInt(prerelease) };
}
