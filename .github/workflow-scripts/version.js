...
const messages = {
  ...
  missing:
    "Found the following packages mentioned in the issue but versions are not found.",
  notFound:
    "Following packages are not found in the issue but they are required.",
};
  ...
  try {
    ...
    requiredPackages.forEach((packageName) => {
      const regexPattern = new RegExp(
        `${packageName.replace("/", "\\/")}:\\s*(\\d+(\\.\\d+)*|Not Found)`
      );
