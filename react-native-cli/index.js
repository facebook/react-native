 ...
 * -v --version - to print current version of react-native installed
 * --verbose - to print logs while init
 * --rn-package <alternative react-native package> - alternative to https://registry.npmjs.org/react-native@latest, possible values:
 *   - "0.22.0-rc1" - react native will be inited with a specific version from npm repo
  ...
  switch (commands[0]) {
    case 'init':
      if (!commands[1]) {
        ...
        init(commands[1], argv.verbose, argv['rn-package']);
      }
      break;
    default:
      console.error(
...
function getInstallPackage(rnPackage) {
  ...
  if (semver.valid(rnPackage)) {
    packageToInstall += '@' + semver.valid(rnPackage);
  } else if (rnPackage) {
...
function runVerbose(root, projectName, rnPackage) {
  var proc = spawn('npm', ['install', '--verbose', '--save', '--cache-min 999999', getInstallPackage(rnPackage)], {stdio: 'inherit'});
