...
It's strongly suggested to use [nvm](https://nvm.sh) (Node Version Manager), rather than install `yarn` into your operating system's global `node_modules`.
...
1. Install [Homebrew](https://brew.sh), which we'll need so we can avoid overwriting important Ruby files used by the operating system
2. `brew install ruby`, then follow the instructions on screen for adding this new Ruby to your path.
3. `cd packages/rn-tester`
4. Install [Bundler](https://bundler.io/): `gem install bundler`. We use bundler to install the right version of [CocoaPods](https://cocoapods.org/) locally.
5. Install Bundler and CocoaPods dependencies: `bundle install && bundle exec pod install` or `yarn setup-ios-hermes`. In order to use JSC instead of Hermes engine, run: `USE_HERMES=0 bundle exec pod install` or `yarn setup-ios-jsc` instead.
6. Open the generated `RNTesterPods.xcworkspace`. This is not checked in, as it is generated by CocoaPods. Do not open `RNTesterPods.xcodeproj` directly.

If you get an error message on the console similar to `SDK cannot be located`, it means you haven't launched Xcode and [explicitly set](https://stackoverflow.com/questions/68565356/xcrun-error-sdk-iphoneos-cannot-be-located) the `Location` of the commandline tools in Xcode's settings yet. You need to do that and then re-run the last step in the above list. 
