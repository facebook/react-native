...
function getStateProperties(
  ...
  const typeAlias = types[stateTypeName];
  try {
    return typeAlias.right.typeParameters.params[0].properties;
  } catch (e) {
    throw new Error(
      `Failed to find type definition for "${stateTypeName}", please check that you have a valid codegen flow file`,
    );
  }
}
    ...
    if (objectType.id.name === '$ReadOnly') {
      return {
        ...
        properties: flattenProperties(
          ...
          .map(prop => buildStateSchema(prop, types))
          .filter(Boolean),
      };
    }
...
function buildStateSchema(
  ...
  // Discard events, they should not be managed in the state
  ...
  // Discard style, they should not be managed in the state
  if (
    name === 'style' &&
    type === 'GenericTypeAnnotation' &&
...
function verifyStateNotAlreadyDefined(
  ...
  if (foundProp) {
    throw new Error(
      `A property was already defined with the name ${propName} in the State`,
    );
  }
}
  ...
  return typeDefinition
    .map(property => {
      if (property.type === 'ObjectTypeProperty') {
        ...
        return flattenProperties(
          getStateProperties(property.argument.id.name, types),
          types,
        );
      }
    })
    .reduce((acc, item) => {
      if (Array.isArray(item)) {
        item.forEach(state => {
          verifyStateNotAlreadyDefined(acc, state);
        });
        return acc.concat(item);
      } else {
        verifyStateNotAlreadyDefined(acc, item);
        acc.push(item);
        return acc;
      }
    }, [])
