...
describe('extractnativeModuleName', () => {
  test('return filename when it ends with .js', () => {
  ...
  test('return filename when it ends with .ts', () => {
    const filename = '/some_folder/NativeModule.ts';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .tsx', () => {
    const filename = '/some_folder/NativeModule.tsx';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .android.js', () => {
    const filename = '/some_folder/NativeModule.android.js';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .android.ts', () => {
    const filename = '/some_folder/NativeModule.android.ts';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .android.tsx', () => {
    const filename = '/some_folder/NativeModule.android.tsx';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .ios.js', () => {
    const filename = '/some_folder/NativeModule.ios.ts';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .ios.ts', () => {
    const filename = '/some_folder/NativeModule.ios.ts';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .ios.tsx', () => {
    const filename = '/some_folder/NativeModule.ios.tsx';
    const nativeModuleName = extractNativeModuleName(filename);
    expect(nativeModuleName).toBe('NativeModule');
  });
  test('return filename when it ends with .windows.js', () => {
...
describe('createParserErrorCapturer', () => {
  describe("when function doesn't throw", () => {
    test("returns result and doesn't change errors array", () => {
  ...
  describe('when function throws a ParserError', () => {
    test('returns null and adds the error in errors array instead of throwing it', () => {
      const [errors, guard] = createParserErrorCapturer();
  ...
  describe('when function throws another error', () => {
    test("throws the error and doesn't change errors array", () => {
      const [errors, guard] = createParserErrorCapturer();
...
describe('verifyPlatforms', () => {
  test('exclude android given an iOS only module', () => {
  ...
  test('exclude iOS given an android only module', () => {
    let result = verifyPlatforms(
      'NativeSampleTurboModule',
  ...
  test('exclude iOS and android given a Cxx only module', () => {
    let result = verifyPlatforms(
      'NativeSampleTurboModule',
...
describe('visit', () => {
  describe('when the astNode is null', () => {
    test("doesn't call the visitor function", () => {
  ...
  describe('when the astNode is an object', () => {
    ...
    describe("when the astNode doesn't have a string type", () => {
      test('iterates on every values of the astNode', () => {
        const visitorFunction = jest.fn();
        const visitor = {
