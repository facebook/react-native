...
function resolveImportedTypes(
...
): {types: TypeDeclarationMap, sourceMap: {[string]: string}} | void {
  ...
  for (const typeName of Object.keys(imports)) {
    ...
    try {
      ...
      if (sourceTypes[typeName] != null) {
        // Only include types that were explicitly imported from this source
        // module. Including all types from the source file can pull in
        // unrelated types (e.g., generic type aliases) that cause parser
        // failures when the codegen tries to process them.
        const hasteName = extractNativeModuleName(sourceModule);
        for (const importName of Object.keys(imports)) {
          if (
            imports[importName] === sourceModule &&
            sourceTypes[importName] != null
          ) {
            importedTypes[importName] = sourceTypes[importName];
            sourceMap[importName] = hasteName;
          }
        }
        found = true;
      }
  ...
  return found ? {types: importedTypes, sourceMap} : undefined;
}
