...
const {getModules, hasArrayBufferType} = require('./Utils');

const ArrayBufferContractComment = ` *
 * ArrayBuffer ownership and lifetime contract (Android / JNI):
 *
 * For \`Ljava/nio/ByteBuffer;\` *parameters*:
 *   - Synchronous methods receive a direct ByteBuffer that wraps the
 *     JS-owned bytes (zero-copy). The bytes are valid only for the duration
 *     of the call. Implementations MUST consume or copy the bytes before
 *     returning.
 *   - Asynchronous methods (void return or Promise) receive a freshly
 *     allocated direct ByteBuffer whose bytes were copied from JS before
 *     dispatch. Implementations may safely retain the ByteBuffer or read
 *     it from any thread.
 *
 * For \`Ljava/nio/ByteBuffer;\` *return values*:
 *   - The ByteBuffer is held alive by a JNI global ref for the lifetime of
 *     the JS ArrayBuffer. The global ref is released on JS GC, which may
 *     run on any thread; the destructor attaches to the JVM before
 *     releasing.`;
