 ...
 * ArrayBuffer ownership and lifetime contract:
 *
 * For \`jsi::ArrayBuffer\` *parameters*:
 *   - Synchronous methods receive a buffer that wraps the JS-owned bytes
 *     (zero-copy). The bytes are valid only for the duration of the call.
 *     Implementations MUST consume or copy the bytes before returning.
 *   - Asynchronous methods (void return or Promise) receive a buffer whose
 *     bytes have been copied into native-owned storage before dispatch.
 *     Implementations may safely retain or read the buffer at any later time.
 *
 * For \`jsi::ArrayBuffer\` *return values*:
 *   - The native-owned source bytes are pinned for the lifetime of the JS
 *     ArrayBuffer (via JNI global ref on Android, ARC retain on iOS).
 *   - The release callback may run on any thread (including the JS GC
 *     thread). Implementations MUST ensure the source is safe to release
 *     from any thread.`;
