...
import {getAreEnumMembersInteger} from './Utils';
...
function FileTemplate(
  ...
  const {packageName, className, jsName, methods, imports, enums} = config;
  return `
/**
...
function EnumTemplate({
  ...
  public enum ${enumName} {
    ...
    static @Nullable ${enumName} fromRawValue(${nativeEnumMemberType} rawValue) {
      ${staticConstructor}
    }

    private final ${nativeEnumMemberType} rawValue;

    ${enumName}(${nativeEnumMemberType} rawValue) {
      this.rawValue = rawValue;
    }

    public ${nativeEnumMemberType} getRawValue() {
      return rawValue;
    }
  }
`;
...
function generateEnumStaticConstructor({
  members,
  nativeEnumMemberType,
  enumName,
  imports,
...
type NativeEnumMemberValueType = 'String' | 'int' | 'double';
...
function generateEnum(
  ...
  const values = members
    .map(member => {
      const memberValue =
        nativeEnumMemberType === 'String' ? `"${member.value}"` : member.value;
      return `${member.name}(${memberValue})`;
    })
    .join(',\n    ');

  const staticConstructor = generateEnumStaticConstructor({
    members,
    nativeEnumMemberType,
    enumName,
    imports,
...
function createEnums(
  ...
  const enums = Object.entries(enumMap)
...
module.exports = {
  generate(
    ...
    Object.keys(nativeModules).forEach(hasteModuleName => {
      ...
      const enums = createEnums(moduleName, enumMap, resolveAlias, imports);
