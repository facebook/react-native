...
function generateCode(outputPath, schemaInfo, includesGeneratedCode, platform) {
  ...
  fs.cpSync(tmpOutputDir, outputPath, {recursive: true});
  console.log(`[Codegen] Generated artifacts: ${outputPath}`);
}
...
function generateRNCoreComponentsIOS(projectRoot /*: string */) /*: void*/ {
  ...
  const coreSpec = findProjectRootLibraries(pkgJson, projectRoot).filter(
  ...
  const outputPath = reactNativeCoreLibraryOutputPath(
    'FBReactNativeSpec',
    platform,
  );
  const coreSchemaInfo = generateSchemaInfo(coreSpec, ios);
  generateCode(outputPath, coreSchemaInfo, false, platform);
}
