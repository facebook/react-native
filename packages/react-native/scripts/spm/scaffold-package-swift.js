...
// Header line written at the top of every scaffolded file. Must NOT contain
// the autolinker's AUTOGEN_MARKER ('// AUTO-GENERATED by scripts/generate-spm-autolinking.js')
// — that marker is what the autolinker uses to tell self-managed deps from
// its own synth output. Our marker is recognized by the scaffolder itself
// (skip-rule), but invisible to the autolinker (treated as self-managed).
...
function emitScaffoldedPackageSwift(
  ...
  return `${SCAFFOLDER_MARKER}${slotComment}
...
function scaffoldPackageSwiftForDep(
  ...
  if (fs.existsSync(pkgSwiftPath)) {
    ...
    if (existing.includes(SCAFFOLDER_MARKER)) {
      if (!ctx.force) {
        // Even without --force we DO want to regenerate if the slot label
        // in the existing file differs — that's the manifest-hash bump
        // mechanism. Compare the slot line.
      ...
      // Fall through: regenerate (force, or slot changed).
    } else {
