...
`setup-apple-spm.js` orchestrates 5 steps:
...
up to date without requiring manual re-runs of `setup-apple-spm.js`.
...
| Build fails after clean with module map errors | Run `setup-apple-spm.js --clean` (not just delete), then reopen Xcode |
| Stale SPM state or corrupted build | Run `setup-apple-spm.js --clean` to reset and rebuild all generated state |
