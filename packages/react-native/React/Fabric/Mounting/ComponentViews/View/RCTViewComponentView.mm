...
@implementation RCTViewComponentView {
  ...
  NSMutableArray<UIView *> *_accessibleElements;
}
...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  ...
  if (oldViewProps.accessibilityElements != newViewProps.accessibilityElements) {
    _accessibleElements = [NSMutableArray new];
    for (const std::string &childId : newViewProps.accessibilityElements) {
      UIView *viewWithMatchingNativeId = [RCTViewFinder findView:self withNativeId:RCTNSStringFromString(childId)];
      if (viewWithMatchingNativeId) {
        [_accessibleElements addObject:viewWithMatchingNativeId];
      }
    }
  }
