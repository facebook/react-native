...
{
  // We can't use the RCTIsNewArchEnabled() function because the RCTRegisterModule() function
  // is called before the RCTIsNewArchEnabled() is initialized.
#if RCT_NEW_ARCH_ENABLED
  if (![getCoreModuleClasses() containsObject:[moduleClass description]]) {
    RCTLogWarn(
        @"The %@ module is registering using a RCT_EXPORT_MODULE. That's a Legacy Architecture API. Please migrate to the new approach as described in the https://reactnative.dev website",
        moduleClass);
  }
#endif
