...
class IdleTaskRef : public jsi::NativeState {
 public:
  ...
  ~IdleTaskRef() noexcept override = default;
...
jsi::Function makeTimeRemainingFunction(
  ...
  return jsi::Function::createFromHostFunction(
      ...
      0,
      [runtimeScheduler, deadline](
          jsi::Runtime& runtime,
          const jsi::Value& /* unused */,
          const jsi::Value* /* unused */,
          size_t /* unused */) {
        if (runtimeScheduler->getShouldYield()) {
          return jsi::Value(runtime, 0.0);
        }

        auto now = runtimeScheduler->now();
        double remainingTime = static_cast<double>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                deadline - now)
                .count());
        return jsi::Value(runtime, std::max(remainingTime, 0.0));
      });
}
