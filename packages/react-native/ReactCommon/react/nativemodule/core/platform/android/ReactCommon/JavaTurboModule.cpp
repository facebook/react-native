...
class JByteBufferMutableBuffer final : public jsi::MutableBuffer {
 public:
  ...
  ~JByteBufferMutableBuffer() override {
    // byteBuffer_ must always hold a live global ref: construction requires
    // one and copy/move are deleted. A null ref here means the buffer was
    // double-destroyed or its memory was corrupted before destruction.
    react_native_assert(byteBuffer_);
...
struct JNIArgs {
  ...
  ~JNIArgs() {
    ...
    for (auto globalRef : globalRefs) {
      // A null slot means makeGlobalIfNecessary failed silently or the
      // vector was mutated between population and destruction. Both would
      // leak the corresponding LocalRef on the original JNI frame.
      react_native_assert(globalRef != nullptr);
...
JNIArgs convertJSIArgsToJNIArgs(
  ...
  auto makeGlobalIfNecessary = [&](jobject obj) {
    if (valueKind == VoidKind || valueKind == PromiseKind) {
      ...
      // NewGlobalRef returns nullptr if obj was null or the JVM is out of
      // global ref capacity. Either case would mean the JNIArgs destructor
      // sees a null slot and the async dispatch reads a dangling jvalue.
      react_native_assert(globalObj != nullptr);
  ...
  for (unsigned int argIndex = 0; argIndex < count; argIndex += 1) {
    ...
    if ((arg->isNull() && type != "Lcom/facebook/react/bridge/Dynamic;") ||
      ...
      if (valueKind == VoidKind || valueKind == PromiseKind) {
        ...
        react_native_assert(byteBuffer->getDirectSize() >= size);
