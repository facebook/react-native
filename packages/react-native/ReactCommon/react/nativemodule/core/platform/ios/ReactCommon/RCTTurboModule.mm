...
// NSMutableData subclass that wraps an externally-owned, fixed-size buffer
// (the JS ArrayBuffer's storage) with true zero-copy mutation semantics.
//
// +[NSMutableData dataWithBytesNoCopy:length:freeWhenDone:NO] does not
// guarantee that -mutableBytes returns the wrapped pointer: the default
// implementation may copy the bytes into an internally-owned buffer on the
// first mutation request, so writes through -mutableBytes never reach the
// original storage. This subclass overrides the NSData/NSMutableData
// primitives so reads and writes go directly to the wrapped pointer; any
// attempt to resize raises, since the JS ArrayBuffer is fixed-size.
//
// The wrapped buffer must outlive any access to this object.
@interface RCTJSArrayBufferData : NSMutableData
+ (instancetype)dataWithExternalBytes:(void *)bytes length:(NSUInteger)length;
@end

@implementation RCTJSArrayBufferData {
  void *_bytes;
  NSUInteger _length;
}
...
+ (instancetype)dataWithExternalBytes:(void *)bytes length:(NSUInteger)length
{
  RCTJSArrayBufferData *data = [[RCTJSArrayBufferData alloc] init];
  data->_bytes = bytes;
  data->_length = length;
  return data;
}
...
- (void)setLength:(NSUInteger)length
{
  if (length != _length) {
    [NSException raise:NSInvalidArgumentException
                format:
                    @"RCTJSArrayBufferData wraps a fixed-size JS ArrayBuffer (length %lu); "
                    @"cannot resize to %lu",
...
class NSMutableDataMutableBuffer final : public jsi::MutableBuffer {
 public:
  ...
  {
    // A nil source would expose a dangling pointer to JS as soon as -data() is
    // called. Catch the construction-time bug rather than a JSI crash later.
    RCTAssert(data != nil, @"NSMutableDataMutableBuffer requires a non-nil NSMutableData");
  }
    ...
    BOOL isSync)
{
  ...
  if (value.isObject()) {
    ...
    if (o.isArrayBuffer(runtime)) {
      ...
      if (isSync) {
        // Codegen guarantees ArrayBuffer args only appear in sync-dispatched methods, so zero-copy is always safe.
        // RCTJSArrayBufferData (vs. +dataWithBytesNoCopy:) ensures -mutableBytes returns the wrapped pointer
        // so in-place writes by the module are visible to JS.
        return [RCTJSArrayBufferData dataWithExternalBytes:(void *)ab.data(runtime) length:ab.size(runtime)];
      }
