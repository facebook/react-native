    ...
    bool isSync)
{
  ...
  if ([argumentType isEqualToString:@"RCTResponseSenderBlock"]) {
    ...
    RCTResponseSenderBlock arg = (RCTResponseSenderBlock)TurboModuleConvertUtils::convertJSIValueToObjCObject(
        runtime, jsiArg, jsInvoker_, YES, isSync);
    if (arg != nullptr) {
      [retainedObjectsForInvocation addObject:arg];
    }
    [inv setArgument:&arg atIndex:(index) + 2];
  ...
  if ([argumentType isEqualToString:@"RCTResponseErrorBlock"]) {
    ...
    RCTResponseSenderBlock senderBlock = (RCTResponseSenderBlock)TurboModuleConvertUtils::convertJSIValueToObjCObject(
        runtime, jsiArg, jsInvoker_, YES, isSync);
