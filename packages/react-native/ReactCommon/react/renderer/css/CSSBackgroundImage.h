...
namespace facebook::react {
  ...
  enum class CSSLinearGradientDirectionKeyword {
  ToTopLeft,
  ToTopRight,
  ToBottomLeft,
  ToBottomRight,
  ...
  auto primaryResult = parser.consumeComponentValue<std::optional<CSSKeyword>>(
      [](const CSSPreservedToken& token) -> std::optional<CSSKeyword> {
        if (token.type() == CSSTokenType::Ident) {
          ...
          if (hash == fnv1a("top")) return CSSKeyword::Top;
          if (hash == fnv1a("bottom")) return CSSKeyword::Bottom;
          if (hash == fnv1a("left")) return CSSKeyword::Left;
          if (hash == fnv1a("right")) return CSSKeyword::Right;
        }
        return {};
      }
  );
...
std::optional<CSSColorHint> parseCSSColorHint(CSSSyntaxParser& parser) {
  auto position = parseNextCSSValue<CSSLengthPercentage>(parser);
  if (std::holds_alternative<CSSLength>(position)) {
    return CSSColorHint{std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(position)}};
  } else if (std::holds_alternative<CSSPercentage>(position)) {
    return CSSColorHint{std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(position)}};
  }
  return {};
}
...
struct CSSDataTypeParser<CSSColorHint> {
  static constexpr auto consume(CSSSyntaxParser& parser) -> std::optional<CSSColorHint> {
...
struct CSSColorStop {
  ...
  bool operator==(const CSSColorStop& rhs) const {
    ...
    if (startPosition.has_value() != rhs.startPosition.has_value()) return false;
    if (startPosition.has_value()) {
      if (startPosition->index() != rhs.startPosition->index()) return false;
      if (*startPosition != *rhs.startPosition) return false;
    }
    
    if (endPosition.has_value() != rhs.endPosition.has_value()) return false;
    if (endPosition.has_value()) {
      if (endPosition->index() != rhs.endPosition->index()) return false;
      if (*endPosition != *rhs.endPosition) return false;
    }
...
std::optional<CSSColorStop> parseCSSColorStop(CSSSyntaxParser& parser) {
  ...
  if (std::holds_alternative<CSSLength>(startPosition)) {
    colorStop.startPosition = std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(startPosition)};
  } else if (std::holds_alternative<CSSPercentage>(startPosition)) {
    colorStop.startPosition = std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(startPosition)};
  }
  
  if (colorStop.startPosition) {
    // Try to parse second optional position (supports both lengths and percentages)
    ...
    if (std::holds_alternative<CSSLength>(endPosition)) {
      colorStop.endPosition = std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(endPosition)};
    } else if (std::holds_alternative<CSSPercentage>(endPosition)) {
      colorStop.endPosition = std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(endPosition)};
    }
  }
  return colorStop;
}
...
std::pair<std::vector<std::variant<CSSColorStop, CSSColorHint>>, int> 
...
enum class CSSRadialGradientSizeKeyword {
  ClosestSide,
  ClosestCorner,
  FarthestSide,
  FarthestCorner,
...
struct CSSDataTypeParser<CSSRadialGradientExplicitSize> {
  static constexpr auto consume(
      CSSSyntaxParser& syntaxParser) -> std::optional<CSSRadialGradientExplicitSize> {
    
    auto sizeX = parseNextCSSValue<CSSLength, CSSPercentage>(syntaxParser);
    if (!std::holds_alternative<CSSLength>(sizeX) && !std::holds_alternative<CSSPercentage>(sizeX)) {
      return {};
    }
    
    syntaxParser.consumeWhitespace();
    
    auto sizeY = parseNextCSSValue<CSSLength, CSSPercentage>(syntaxParser);
  ...
  static constexpr auto consumeFunctionBlock(
    ...
    if (std::holds_alternative<CSSRadialGradientSizeKeyword>(sizeKeywordResult)) {
      ...
      if (std::holds_alternative<CSSRadialGradientExplicitSize>(explicitSizeResult)) {
        ...
        // check if it's a single size (both X and Y are the same), we use it to set shape to circle
    ...
    if (hasExplicitSingleSize && hasExplicitShape && gradient.shape.value() == CSSRadialGradientShape::Ellipse) {
      // if a single size is explicitly set and the shape is an ellipse do not apply any gradient. Same as web.
      return {};
    }
    ...
    if (atResult) {
      parser.consumeWhitespace();
      std::vector<std::optional<std::variant<CSSLength, CSSPercentage, CSSKeyword>>> positionKeywordValues;
      for (int i = 0; i < 2; i++) {
        ...
        if (positionKeyword) {
          ...
          for (const auto& existingValue : positionKeywordValues) {
            if (existingValue.has_value() && std::holds_alternative<CSSKeyword>(*existingValue)) {
              if (std::get<CSSKeyword>(*existingValue) == *positionKeyword) {
                return {};
              }
            }
          }
          positionKeywordValues.push_back(*positionKeyword);
          keywordFound = true;
        }
        ...
        std::optional<std::variant<CSSLength, CSSPercentage, CSSKeyword>> value;
        if (std::holds_alternative<CSSLength>(lengthPercentageValue)) {
          value = std::variant<CSSLength, CSSPercentage, CSSKeyword>{std::get<CSSLength>(lengthPercentageValue)};
        } else if (std::holds_alternative<CSSPercentage>(lengthPercentageValue)) {
          value = std::variant<CSSLength, CSSPercentage, CSSKeyword>{std::get<CSSPercentage>(lengthPercentageValue)};
        }

        if (value) {
          positionKeywordValues.push_back(value);
          valueFound = true;
        }
      ...
      if (positionKeywordValues.size() == 0) {
        return {};
      }
      ...
      if (positionKeywordValues.size() == 1) {
        ...
        if (value.has_value() && std::holds_alternative<CSSKeyword>(*value)) {
          auto keyword = std::get<CSSKeyword>(*value);
          if (keyword == CSSKeyword::Left) {
            position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{50.0f}};
            position.left = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
          } else if (keyword == CSSKeyword::Right) {
            position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{50.0f}};
            position.left = std::variant<CSSLength, CSSPercentage>{CSSPercentage{100.0f}};
          } else if (keyword == CSSKeyword::Top) {
            position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
            position.left = std::variant<CSSLength, CSSPercentage>{CSSPercentage{50.0f}};
          } else if (keyword == CSSKeyword::Bottom) {
            position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{100.0f}};
            position.left = std::variant<CSSLength, CSSPercentage>{CSSPercentage{50.0f}};
        ...
        } else if (value.has_value() && (std::holds_alternative<CSSLength>(*value) || std::holds_alternative<CSSPercentage>(*value))) {
          if (std::holds_alternative<CSSLength>(*value)) {
            position.left = std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(*value)};
          } else {
            position.left = std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(*value)};
          }
          position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{50.0f}};
        } else {
          return {};
        ...
        if (value1.has_value() && value2.has_value() && std::holds_alternative<CSSKeyword>(*value1) && std::holds_alternative<CSSKeyword>(*value2)) {
          ...
          if (isHorizontal(keyword1) && isVertical(keyword2)) {
            ...
            if (keyword1 == CSSKeyword::Left) {
              ...
              position.right = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
            ...
            if (keyword2 == CSSKeyword::Top) {
              ...
              position.bottom = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
        ...
        // 3. [ left | center | right | <length-percentage> ] [ top | center | bottom | <length-percentage> ]
          ...
          if (value1.has_value() && std::holds_alternative<CSSKeyword>(*value1)) {
            auto keyword1 = std::get<CSSKeyword>(*value1);
            if (keyword1 == CSSKeyword::Left) {
              position.left = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
            } else if (keyword1 == CSSKeyword::Right) {
          ...
          } else if (value1.has_value() && (std::holds_alternative<CSSLength>(*value1) || std::holds_alternative<CSSPercentage>(*value1))) {
            if (std::holds_alternative<CSSLength>(*value1)) {
              position.left = std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(*value1)};
            } else {
              position.left = std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(*value1)};
            }
          } else {
            return {};
          }

          if (value2.has_value() && std::holds_alternative<CSSKeyword>(*value2)) {
            auto keyword2 = std::get<CSSKeyword>(*value2);
            if (keyword2 == CSSKeyword::Top) {
              position.top = std::variant<CSSLength, CSSPercentage>{CSSPercentage{0.0f}};
            } else if (keyword2 == CSSKeyword::Bottom) {
          ...
          } else if (value2.has_value() && (std::holds_alternative<CSSLength>(*value2) || std::holds_alternative<CSSPercentage>(*value2))) {
            if (std::holds_alternative<CSSLength>(*value2)) {
              position.top = std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(*value2)};
            } else {
              position.top = std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(*value2)};
            }
          } else {
            return {};
          }
      ...
      // 4. [ [ left | right ] <length-percentage> ] && [ [ top | bottom ] <length-percentage> ]
        ...
        if (!value3.has_value() || !std::holds_alternative<CSSKeyword>(*value3)) {
          return {};
        }

        if (!value2.has_value() || (!std::holds_alternative<CSSLength>(*value2) && !std::holds_alternative<CSSPercentage>(*value2))) {
          return {};
        }

        if (!value4.has_value() || (!std::holds_alternative<CSSLength>(*value4) && !std::holds_alternative<CSSPercentage>(*value4))) {
          return {};
        }

        auto parsedValue2 = std::holds_alternative<CSSLength>(*value2) ? std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(*value2)} : std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(*value2)};
        auto parsedValue4 = std::holds_alternative<CSSLength>(*value4) ? std::variant<CSSLength, CSSPercentage>{std::get<CSSLength>(*value4)} : std::variant<CSSLength, CSSPercentage>{std::get<CSSPercentage>(*value4)};
        auto keyword1 = std::get<CSSKeyword>(*value1);
    ...
    auto [items, colorStopCount] = parseGradientColorStopsAndHints(parser);
...
struct CSSDataTypeParser<CSSLinearGradientFunction> {
  static constexpr auto consumeFunctionBlock(
    ...
    auto parsedDirection = parseLinearGradientDirection(parser);
    if (!parsedDirection) {
      return {};
    }
