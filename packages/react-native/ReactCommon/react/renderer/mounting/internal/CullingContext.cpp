  ...
  if (ReactNativeFeatureFlags::enableViewCulling()) {
    if (auto scrollViewShadowNode =
      ...
      if (scrollViewShadowNode->getConcreteProps().yogaStyle.overflow() !=
        ...
        if (outsetRatio > 0) {
          auto xOutset =
              std::floor(cullingContext.frame.size.width * outsetRatio);
          auto yOutset =
              std::floor(cullingContext.frame.size.height * outsetRatio);
          cullingContext.frame.origin.x -= xOutset;
          cullingContext.frame.origin.y -= yOutset;
          cullingContext.frame.size.width += 2 * xOutset;
          cullingContext.frame.size.height += 2 * yOutset;
        }
