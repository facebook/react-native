...
MapBuffer convertCascadedEdges(CascadedRectangleEdges<T> const &edges) {
  ...
  putOptionalFloat(builder, EDGE_INLINE, optionalFromValue(edges.inline));
  putOptionalFloat(
      builder, EDGE_INLINE_END, optionalFromValue(edges.inlineEnd));
  putOptionalFloat(
      builder, EDGE_INLINE_START, optionalFromValue(edges.inlineStart));
