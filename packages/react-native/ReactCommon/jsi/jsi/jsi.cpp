...
void Runtime::setRuntimeData(
  ...
  // If not done already, install a host object on the global object of the
  // runtime. This host object is responsible for releasing the runtime's
  // entry from the global custom data map when the runtime is destroyed.
  auto globalObj = global();
  if (!globalObj.hasProperty(*this, "_jsiRuntimeDataCleanUp")) {
    Object ho = Object::createFromHostObject(
        *this, std::make_shared<RemoveRuntimeDataHostObject>(this));
    globalObj.setProperty(*this, "_jsiRuntimeDataCleanUp", ho);
  }
  auto& runtimeDataGlobal = getRuntimeDataGlobal();
  std::lock_guard<std::mutex> lock(runtimeDataGlobal.mutex_);
