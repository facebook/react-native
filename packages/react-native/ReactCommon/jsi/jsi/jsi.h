...
class JSI_EXPORT ISerialization : public ICast {
 public:
  ...
  /// entirely and make the serialized objects in the current runtime. Any
  /// transferred values in the serialized object will be owned by the current
  /// runtime. Returns an Array containing the deserialized values, where the
  /// first element is the value passed into serializeWithTransfer, followed by
