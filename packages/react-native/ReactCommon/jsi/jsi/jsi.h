...
class JSI_EXPORT Runtime {
 public:
  ...
  /// destroyed, the runtime will release its ownership of the held object.
  ...
  const std::shared_ptr<void>& getRuntimeData(const UUID& uuid);
 ...
 protected:
  ...
  /// Stores the pointer \p data with the \p uuid in the runtime. This can be
  /// used to store some custom data within the runtime. When the runtime is
  /// destroyed, the runtime will release its ownership by calling \p deleter.
