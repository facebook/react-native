...
class JSI_EXPORT ISerialization : public ICast {
 public:
  ...
  /// of \p transferList, it will transfer everything at index [0, length - 1].
  /// It returns a unique pointer of an opaque Serialized object that can be
  /// deserialized once only by deserializeWithTransfer. The lifetime of the
  /// Serialized object is not tied to the lifetime of the original object.
  ...
  /// entirely to transfer the object to the current runtime. Returns an Array
  /// containing the deserialized values, where the first element is the value
  /// passed into serializeWithTransfer, followed by all transferred values.
