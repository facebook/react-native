...
/// UUID version 1 implementation, used to identify a specific interface.
...
struct JSI_EXPORT ICast {
  ...
  /// pointer.
  /// The lifetime of the object is expected to be managed by smart pointers and
  /// destroyed after all referencing pointers are destroyed.
  virtual ICast* castInterface(const UUID& interfaceUUID) = 0;

 protected:
  /// Protected destructor to prevent delete calls on the interface. The  The
  /// destructor is non-virtual to reduce the vtable complexity from
  /// inheritance.
  ~ICast() = default;
};
...
std::shared_ptr<U> dynamic_interface_cast(std::shared_ptr<T>& ptr) {
