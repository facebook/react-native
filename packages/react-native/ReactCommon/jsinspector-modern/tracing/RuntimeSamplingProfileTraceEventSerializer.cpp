...
RuntimeSamplingProfileTraceEventSerializer::serializeAndBuffer(
  ...
  const std::string& runtimeName = profile.getRuntimeName();
  RuntimeSamplingProfile::Process process = profile.getProcess();
  const RuntimeSamplingProfile::Thread& thread = profile.getThread();

  performanceTracer.reportProcess(process.getId(), runtimeName);
  performanceTracer.reportThread(
      thread.getId(), thread.getName(), process.getId());
  ...
  uint16_t profileId = performanceTracer.reportRuntimeProfile(
      process.getId(), thread.getId(), tracingStartUnixTimestamp);
