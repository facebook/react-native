...
class NetworkingModuleTest {
  ...
  fun prepareModules() {
    ...
    requestBodyUtil
      .`when`<InputStream> {
        RequestBodyUtil.getFileInputStream(
          any(ReactContext::class.java),
    ...
    requestBodyUtil
      .`when`<ProgressRequestBody> {
        RequestBodyUtil.createProgressRequest(
          ...
          any(ProgressListener::class.java)
        )
      }
      .thenCallRealMethod()
  ...
  private fun verifyErrorEmit(context: ReactApplicationContext, requestId: Int) {
    ...
    verify(context)
      .emitDeviceEvent(eq("didCompleteNetworkResponse"), captor.capture())
  ...
  fun testMultipartPostRequestBody() {
    ...
    whenever(
      multipartBuilder.addPart(
        any(Headers::class.java),
        anyOrNull(RequestBody::class.java)
      )
    )
    ...
    assertThat(bodyRequestBody[0].contentLength())
      .isEqualTo("locale".toByteArray().size.toLong())
    ...
    assertThat(bodyRequestBody[1].contentLength())
      .isEqualTo("imageUri".toByteArray().size.toLong())
  }
