...
class AppStateModuleTest {
  ...
  fun testGetCurrentAppState() {
    ...
    val activeAppStateMap = Arguments.createMap().apply { putString("app_state", AppStateModule.APP_STATE_ACTIVE) }
    verify(successCallbackMock, times(1)).invoke(activeAppStateMap)

    appStateModule.onHostPause()
    appStateModule.getCurrentAppState(successCallbackMock, errorCallbackMock)

    val backgroundAppStateMap = Arguments.createMap().apply { putString("app_state", AppStateModule.APP_STATE_BACKGROUND) }
  ...
  fun testOnHostResume() {
    ...
    assertThat(eventArgumentsCaptor.value.getString("app_state")).isEqualTo(AppStateModule.APP_STATE_ACTIVE)
  }
  ...
  fun testOnHostPause() {
    ...
    assertThat(eventNameCaptor.value).isEqualTo("appStateDidChange")
    assertThat(eventArgumentsCaptor.value.getString("app_state")).isEqualTo(AppStateModule.APP_STATE_BACKGROUND)
  }
