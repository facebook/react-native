...
class JSDebuggerWebSocketClientTest {
    ...
    fun test_loadBundle_ShouldSendCorrectMessage() {
        val cb = PowerMockito.mock(JSDebuggerCallback::class.java)
        val client = PowerMockito.spy(JSDebuggerWebSocketClient())
        val injectedObjects = HashMap<String, String>()
        injectedObjects["key1"] = "value1"
        injectedObjects["key2"] = "value2"
    ...
    fun test_onMessage_WithInvalidContentType_ShouldNotTriggerCallbacks() {
        ...
        val client = PowerMockito.spy(JSDebuggerWebSocketClient())
        client.onMessage(
            PowerMockito.mock(WebSocket::class.java),
            "{\"replyID\":0, \"result\":\"OK\"}".encodeUtf8()
        )
        PowerMockito.verifyPrivate(client, Mockito.never())
            .invoke(
                "triggerRequestSuccess", ArgumentMatchers.anyInt(), ArgumentMatchers.nullable(
                    String::class.java
                )
            )
        PowerMockito.verifyPrivate(client, Mockito.never())
    ...
    fun test_onMessage_WithError_ShouldCallAbort() {
        ...
        PowerMockito.verifyPrivate(client).invoke(
            "abort", ArgumentMatchers.eq("BOOM"), ArgumentMatchers.isA(
                JavascriptException::class.java
            )
        )
    }
