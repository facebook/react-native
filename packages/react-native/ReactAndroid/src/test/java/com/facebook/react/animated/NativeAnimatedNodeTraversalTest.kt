...
class NativeAnimatedNodeTraversalTest {
  ...
  companion object {
    private val FRAME_LEN_NANOS: Long = 1000000000L / 60L
    private val INITIAL_FRAME_TIME_NANOS: Long = 14599233201256L /* random */
  }
  ...
  fun setUp() {
    ...
    whenever(reactApplicationContextMock.getCatalystInstance()).thenAnswer {
      catalystInstanceMock
    }
    whenever(reactApplicationContextMock.getNativeModule(UIManagerModule::class.java))
      .thenAnswer { uiManagerMock }
    ...
    whenever(uiManagerMock.getDirectEventNamesResolver()).thenAnswer {
      object : UIManagerModule.CustomEventNamesResolver {
        override fun resolveCustomEventName(eventName: String): String {
          ...
          if (directEventTypes != null) {
            val customEventType: Map<String, String>? = directEventTypes[eventName] as? Map<String, String>?
            if (customEventType != null) {
              return customEventType["registrationName"] ?: eventName
            }
          }
          return eventName
        }
    ...
    whenever(uiManagerMock.resolveCustomDirectEventName(any(String::class.java))).thenAnswer { invocation
      ->
  ...
  /**
   * Generates a simple animated nodes graph and attaches the props node to a given {@param
   * viewTag} Parameter {@param opacity} is used as a initial value for the "opacity" attribute.
   *
   * <p>Nodes are connected as follows (nodes IDs in parens): ValueNode(1) -> StyleNode(2) ->
   * PropNode(3)
   */
  private fun createSimpleAnimatedViewWithOpacity(viewTag: Int, opacity: Double) {
  ...
  fun performSpringAnimationTestWithConfig(
    ...
    assertThat(stylesCaptor.getValue().getDouble("opacity")).isEqualTo(0.0)
    ...
    for (i in 0 until 3 * 60) {
      ...
      assertThat(java.lang.Math.abs(currentValue - previousValue)).isLessThan(0.12)
  ...
  fun testSpringAnimationLoopsFiveTimes() {
    ...
    for (i in 0 until 3 * 60 * 5) {
      ...
      if (
        didComeToRest &&
        currentValue == 0.0 &&
        Math.abs(Math.abs(currentValue - previousValue) - 1.0) < 0.001
      ) {
        numberOfResets++
      }
      ...
      // reset
      if (!didComeToRest) assertThat(Math.abs(currentValue - previousValue)).isLessThan(0.12)

      // record that the animation did come to rest when it rests on toValue
      didComeToRest =
        Math.abs(currentValue - 1.0) < 0.001 &&
          Math.abs(currentValue - previousValue) < 0.001
      previousValue = currentValue
    }
    // verify that we've reach the final value at the end of animation
    assertThat(previousValue).isEqualTo(1.0)
  ...
  fun testAdditionNode() {
    createAnimatedGraphWithAdditionNode(50, 100.0, 1000.0)
    ...
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(1100.0)

    reset(uiManagerMock)
    nativeAnimatedNodesManager.runUpdates(nextFrameTime())
    verify(uiManagerMock).synchronouslyUpdateViewOnUIThread(eq(50), stylesCaptor.capture())
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(1111.0)
  ...
  /**
   * Verifies that {@link NativeAnimatedNodesManager#runUpdates} updates the view correctly in
   * case when one of the addition input nodes has started animating while the other one has not.
   *
  ...
  fun testViewReceiveUpdatesIfOneOfAnimationHasntStarted() {
    ...
    nativeAnimatedNodesManager.runUpdates(nextFrameTime())
    verify(uiManagerMock).synchronouslyUpdateViewOnUIThread(eq(50), stylesCaptor.capture())
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(1101.0)
  ...
  /**
   * Verifies that {@link NativeAnimatedNodesManager#runUpdates} updates the view correctly in
   * case when one of the addition input nodes animation finishes before the other.
   *
   * <p>We expect that the output of the addition node after one of the animation has finished
   * will take the last value of the animated node and the view will receive updates up until the
   * second animation is over.
   */
  @Test
  ...
  fun testMultiplicationNode() {
    ...
    nativeAnimatedNodesManager.runUpdates(nextFrameTime())
    verify(uiManagerMock).synchronouslyUpdateViewOnUIThread(eq(50), stylesCaptor.capture())
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(5.0)

    reset(uiManagerMock)
    nativeAnimatedNodesManager.runUpdates(nextFrameTime())
    verify(uiManagerMock).synchronouslyUpdateViewOnUIThread(eq(50), stylesCaptor.capture())
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(20.0)
  ...
  /**
   * This test verifies that when {@link NativeAnimatedModule#stopAnimation} is called the
   * animation will no longer be updating the nodes it has been previously attached to and that
   * the animation callback will be triggered with {@code {finished: false}}
   */
  @Test
  ...
  fun testNativeAnimatedEventDoUpdate() {
    ...
    createSimpleAnimatedViewWithOpacity(viewTag, 0.0)
    ...
    assertThat(stylesCaptor.getValue().getDouble("opacity")).isEqualTo(10.0)
  }
  ...
  /**
   * In this test we verify that when value is being tracked we can update destination value in
   * the middle of ongoing animation and the animation will update and animate to the new spot.
   * This is tested using simple 5 frame backed timing animation.
   */
  @Test
  fun testTracking() {
    ...
    createAnimatedGraphWithTrackingNode(1000, 0.0, animationConfig)
    ...
    nativeAnimatedNodesManager.runUpdates(nextFrameTime())
    verify(uiManagerMock).synchronouslyUpdateViewOnUIThread(eq(1000), stylesCaptor.capture())
    assertThat(stylesCaptor.getValue().getDouble("translateX")).isEqualTo(0.0)
   ...
   * consume resources on running the animation that has already completed. Then we verify that
   * when the value updates the animation will resume as expected and the complete again when
   * reaches the end.
   */
  @Test
  ...
  fun testSpringTrackingRetainsSpeed() {
    ...
    createAnimatedGraphWithTrackingNode(1000, 0.0, springConfig)
