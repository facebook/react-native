...
jintArray FabricUIManagerBinding::findNextFocusableElementMetrics(
    jint parentTag,
    jint focusedTag,
    jint direction) {
  /*
   ...
   * [1] - delta
   */
  std::array<int32_t, 2> nodeValues{-1, -1};
  ...
  jintArray result = env->NewIntArray(nodeValuesSize);
  ...
  // Build the jintArray with the updates nodeValues
  env->SetIntArrayRegion(result, 0, nodeValuesSize, nodeValues.data());
