...
interface UIManagerJSI : UIManager, JSIModule

interface UIManager : PerformanceCounter {
    /** Registers a new root view. @Deprecated call startSurface instead  */
    @UiThread
    @ThreadConfined(ThreadConfined.UI)
    @Deprecated("")
    fun <T : View?> addRootView(
        rootView: T, initialProps: WritableMap?, initialUITemplate: String?
    ): Int

    /** Registers a new root view with width and height.  */
    @AnyThread
    fun <T : View?> startSurface(
        rootView: T,
        moduleName: String?,
    ...
    @UiThread
    @ThreadConfined(ThreadConfined.UI)
    fun updateRootLayoutSpecs(
        rootTag: Int, widthMeasureSpec: Int, heightMeasureSpec: Int, offsetX: Int, offsetY: Int
    )
     ...
     *
     * @param reactTag [int] that identifies the view that will receive this command
     * @param commandId [int] command id
     * @param commandArgs [ReadableArray] parameters associated with the command
     */
    fun dispatchCommand(reactTag: Int, commandId: Int, commandArgs: ReadableArray?)
     ...
     * @param commandArgs [ReadableArray] parameters associated with the command
     */
    fun dispatchCommand(reactTag: Int, commandId: String?, commandArgs: ReadableArray?)

    /** @return the [EventDispatcher] object that is used by this class.
     */
    fun <T> getEventDispatcher(): T
     ...
     *
     * @param reactTag [int] that identifies the view that will be updated
     ...
     * @param eventType
     */
    fun sendAccessibilityEvent(reactTag: Int, eventType: Int)
     ...
     * @param listener
     */
    fun addUIManagerEventListener(listener: UIManagerListener?)

    /**
     * Unregister a [UIManagerListener] from this UIManager to stop receiving lifecycle
     * callbacks.
     *
     * @param listener
     */
    fun removeUIManagerEventListener(listener: UIManagerListener?)
     ...
     */
    fun resolveView(reactTag: Int): View?
     ...
     */
    @Deprecated("")
    fun receiveEvent(reactTag: Int, eventName: String?, event: WritableMap?)
     ...
     * @param event parameters
     */
    fun receiveEvent(surfaceId: Int, reactTag: Int, eventName: String?, event: WritableMap?)

    /** Resolves Direct Event name exposed to JS from the one known to the Native side.  */
    @Deprecated("")
    fun resolveCustomDirectEventName(eventName: String?): String?
}
