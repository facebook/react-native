...
public class Spacing {
  private val spacing: FloatArray
  private var valueFlags = 0
  private val defaultValue1: Float
  private var hasAliasesSet = false

  @JvmOverloads
  public constructor(defaultValue: Float = 0f) {
    defaultValue1 = defaultValue
    spacing = newFullSpacingArray()
  }

  public constructor(original: Spacing) {
    defaultValue1 = original.defaultValue1
    spacing = Arrays.copyOf(original.spacing, original.spacing.size)
   ...
   */
  public fun reset() {
    Arrays.fill(spacing, YogaConstants.UNDEFINED)
    hasAliasesSet = false
    valueFlags = 0
  }
  ...
  public fun getWithFallback(spacingType: Int, fallbackType: Int): Float {
    return if (valueFlags and flagsMap[spacingType] != 0) spacing[spacingType]
    else get(fallbackType)
  }
