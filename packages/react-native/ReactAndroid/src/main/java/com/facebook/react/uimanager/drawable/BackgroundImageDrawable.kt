...
internal class BackgroundImageDrawable(
  ...
  override fun draw(canvas: Canvas) {
    ...
    backgroundImageLayers?.let { layers ->
      ...
      for (index in layers.indices.reversed()) {
        ...
        val (tileWidth, tileHeight) = calculateBackgroundImageSize(
  ...
  private fun hasInvalidDimensions(positioningArea: RectF, paintingArea: RectF): Boolean =
      FloatUtil.floatsEqual(positioningArea.width(), 0f) || positioningArea.width() < 0f ||
      FloatUtil.floatsEqual(positioningArea.height(), 0f) || positioningArea.height() < 0f ||
      FloatUtil.floatsEqual(paintingArea.width(), 0f) || paintingArea.width() < 0f ||
      FloatUtil.floatsEqual(paintingArea.height(), 0f) || paintingArea.height() < 0f
  ...
  private fun updatePath() {
    ...
    backgroundPositioningArea = RectF(
