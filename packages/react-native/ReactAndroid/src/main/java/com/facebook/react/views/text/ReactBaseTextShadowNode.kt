...
public constructor(protected var reactTextViewManagerCallback: ReactTextViewManagerCallback? = null) :
  ...
  public open fun setTextBreakStrategy(textBreakStrategy: String?) {
    this.textBreakStrategy = when (textBreakStrategy) {
      null, "highQuality" -> Layout.BREAK_STRATEGY_HIGH_QUALITY
  ...
  public fun setTextShadowOffset(offsetMap: ReadableMap?) {
    ...
    offsetMap?.let {
      if (it.hasKey(PROP_SHADOW_OFFSET_WIDTH) &&
          !it.isNull(PROP_SHADOW_OFFSET_WIDTH)) {
        textShadowOffsetDx = toPixelFromDIP(it.getDouble(PROP_SHADOW_OFFSET_WIDTH))
      }
      if (it.hasKey(PROP_SHADOW_OFFSET_HEIGHT) &&
          !it.isNull(PROP_SHADOW_OFFSET_HEIGHT)) {
  ...
  public fun setTextTransform(textTransform: String?) {
    val textTransformEnum = when (textTransform) {
    ...
    private fun buildSpannedFromShadowNode(
      ...
      while (i < length) {
        ...
        if (child is ReactRawTextShadowNode) {
          child.text?.let {
            sb.append(apply(it, textAttributes.textTransform))
          }
