...
public abstract class ReactPackageTurboModuleManagerDelegate : TurboModuleManagerDelegate {
  ...
  // Lazy Props
  private val packages: List<ReactPackage>? = null
  ...
  private fun initialize(
    ...
    for (reactPackage in packages) {
      if (reactPackage is BaseReactPackage) {
        val baseReactPackage = reactPackage
        val moduleProvider: ModuleProvider =
            ModuleProvider { moduleName: String ->
                baseReactPackage.getModule(
                    moduleName, applicationContext
                )
            }
        moduleProviders.add(moduleProvider)
        packageModuleInfos[moduleProvider] = baseReactPackage.getReactModuleInfoProvider().getReactModuleInfos()
        continue
      }
        ...
        if (shouldSupportLegacyPackages() && reactPackage is LazyReactPackage) {
          ...
          val moduleSpecs: List<ModuleSpec> = lazyPkg.getNativeModules(reactApplicationContext)
          val moduleSpecProviderMap: MutableMap<String?, Provider<out NativeModule>> =
              mutableMapOf()
        ...
        if (shouldSupportLegacyPackages()) {
          ...
          for (module in nativeModules) {
            ...
            val reactModule = moduleClass.getAnnotation<ReactModule>(
              ReactModule::class.java
            )

            val moduleName = if (reactModule != null) reactModule.name else module.name
          ...
          val moduleProvider: ModuleProvider = ModuleProvider { o -> moduleMap[o] }
  ...
  public abstract class Builder {
    ...
    public fun build(): ReactPackageTurboModuleManagerDelegate {
      ...
      val nonNullPackages = requireNotNull(packages) {
