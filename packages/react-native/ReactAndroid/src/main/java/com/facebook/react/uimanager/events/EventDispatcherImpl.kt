...
import com.facebook.react.modules.core.ReactChoreographer.Companion.getInstance
import com.facebook.systrace.Systrace
import com.facebook.systrace.Systrace.beginSection
import com.facebook.systrace.Systrace.endAsyncFlow
import com.facebook.systrace.Systrace.endSection
import com.facebook.systrace.Systrace.startAsyncFlow
...
internal class EventDispatcherImpl(private val reactContext: ReactApplicationContext) :
  ...
  override fun dispatchEvent(event: Event<*>) {
    ...
    synchronized(eventsStagingLock) {
      ...
      startAsyncFlow(Systrace.TRACE_TAG_REACT, event.getEventName(), event.uniqueID)
    }
      ...
      synchronized(eventsToDispatchLock) {
        for (i in eventStaging.indices) {
          ...
          if (lastEventIdx == null) {
            ...
            val coalescedEvent = coalesceRawGenerics(event, lastEvent)
  ...
  private inner class DispatchEventsRunnable : Runnable {
    override fun run() {
      beginSection(Systrace.TRACE_TAG_REACT, "DispatchEventsRunnable")
      try {
        endAsyncFlow(
            Systrace.TRACE_TAG_REACT,
            "ScheduleDispatchFrameCallback",
        ...
        synchronized(eventsToDispatchLock) {
          if (eventsToDispatchSize > 0) {
            ...
            for (eventIdx in 0..<eventsToDispatchSize) {
              ...
              endAsyncFlow(Systrace.TRACE_TAG_REACT, event.getEventName(), event.uniqueID)
  ...
  companion object {
    ...
    /**
     * This function is necessary due to previous usage of Raw generics which is making Kotlin
     * conversions harder
     */
    internal fun coalesceRawGenerics(event1: Event<*>, event2: Event<*>): Event<*> =
        if (event1.timestampMs >= event2.timestampMs) event1 else event2
  }
}
