...
public object PointerEventHelper {
  ...
  public fun getButtonChange(
    ...
    when (changedMask) {
      MotionEvent.BUTTON_PRIMARY -> return 0
      MotionEvent.BUTTON_TERTIARY -> return 1
      MotionEvent.BUTTON_SECONDARY -> return 2
      MotionEvent.BUTTON_BACK -> return 3
      MotionEvent.BUTTON_FORWARD -> return 4
    }
    return -1
  }
  ...
  public fun getW3CPointerType(toolType: Int): String {
    ...
    when (toolType) {
      MotionEvent.TOOL_TYPE_FINGER -> return POINTER_TYPE_TOUCH

      MotionEvent.TOOL_TYPE_STYLUS -> return POINTER_TYPE_PEN

      MotionEvent.TOOL_TYPE_MOUSE -> return POINTER_TYPE_MOUSE
    }
  ...
  public fun isListening(view: View?, event: EVENT): Boolean {
    ...
    when (event) {
      ...
      EVENT.CLICK,
      EVENT.CLICK_CAPTURE -> return true
      else -> {
        val pointerEvents = view.getTag(R.id.pointer_events) as? Int
        if (pointerEvents != null) {
          return (pointerEvents and (1 shl event.ordinal)) != 0
        }
        return false
      }
    }
  ...
  public fun getEventCategory(pointerEventType: String?): Int {
    ...
    when (pointerEventType) {
      POINTER_DOWN,
      POINTER_CANCEL,
      POINTER_UP -> return EventCategoryDef.DISCRETE
      POINTER_MOVE,
      POINTER_ENTER,
      POINTER_LEAVE,
      POINTER_OVER,
      POINTER_OUT -> return EventCategoryDef.CONTINUOUS
    }
    return EventCategoryDef.UNSPECIFIED
  }
