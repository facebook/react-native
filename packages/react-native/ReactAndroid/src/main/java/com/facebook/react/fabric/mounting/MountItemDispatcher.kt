...
internal class MountItemDispatcher(
  ...
  private val viewCommandMountItems: ConcurrentLinkedQueue<DispatchCommandMountItem> =
      ConcurrentLinkedQueue<DispatchCommandMountItem>()
  private val mountItems: ConcurrentLinkedQueue<MountItem> = ConcurrentLinkedQueue<MountItem>()
  private val preMountItems: ConcurrentLinkedQueue<MountItem> = ConcurrentLinkedQueue<MountItem>()
  ...
  private fun dispatchMountItems() {
    ...
    val viewCommandMountItemsToDispatch = andResetViewCommandMountItems
    val mountItemsToDispatch = andResetMountItems
    ...
    // first
    andResetPreMountItems?.let { preMountItems ->
      Systrace.beginSection(
  ...
  private fun dispatchPreMountItemsImpl(deadline: Long) {
    ...
    try {
      while (true) {
        ...
        val preMountItemToDispatch: MountItem? = preMountItems.poll()
  ...
  @get:ThreadConfined(UI)
  @get:UiThread
  private val andResetViewCommandMountItems: List<DispatchCommandMountItem>?
    get() = drainConcurrentItemQueue(viewCommandMountItems)

  @get:ThreadConfined(UI)
  @get:UiThread
  private val andResetMountItems: List<MountItem>?
    get() = drainConcurrentItemQueue(mountItems)

  @get:ThreadConfined(UI)
  @get:UiThread
  private val andResetPreMountItems: List<MountItem>?
    get() = drainConcurrentItemQueue(preMountItems)
  ...
  private companion object {
    ...
    private val FRAME_TIME_NS: Long = (1000000000 / 60).toLong()

    private fun <E> drainConcurrentItemQueue(queue: ConcurrentLinkedQueue<E>): List<E>? {
