...
public class ReactInstanceManagerBuilder {
  ...
  public fun build(): ReactInstanceManager {
    val application = checkNotNull(this.application) { "Application property has not been set with this builder" }
    ...
    return ReactInstanceManager(
        ...
        javaScriptExecutorFactory ?: getDefaultJSExecutorFactory(appName, deviceName, application.applicationContext),
  ...
  private fun getDefaultJSExecutorFactory(
      appName: String, deviceName: String, applicationContext: Context
    ...
    if (jsEngineResolutionAlgorithm == null) {
      ...
      } catch (ignoredHermesError: UnsatisfiedLinkError) {
        try {
          JSCExecutor.loadLibrary()
          return JSCExecutorFactory(appName, deviceName)
        } catch (jscError: UnsatisfiedLinkError) {
          FLog.e(
              TAG,
          ...
          val message = jscError.message ?: ""
          if (message.contains("__cxa_bad_typeid")) {
              throw jscError
          }
          return null
        }
