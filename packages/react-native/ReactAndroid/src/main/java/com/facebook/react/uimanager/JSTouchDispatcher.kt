...
public class JSTouchDispatcher(private val viewGroup: ViewGroup) {
  ...
  public fun onChildStartedNativeGesture(
      androidEvent: MotionEvent, eventDispatcher: EventDispatcher
  ) {
  ...
  /**
    ...
    * reacts for an incoming android native touch events ([MotionEvent]) and calls into [com.facebook.react.uimanager.events.EventDispatcher] when appropriate. It uses [com.facebook.react.uimanager.TouchTargetHelper.findTargetTagAndCoordinatesForTouch] helper
    * method for figuring out a react view ID in the case of ACTION_DOWN event (when the gesture
    * starts).
    */
  public fun handleTouchEvent(ev: MotionEvent, eventDispatcher: EventDispatcher, reactContext: ReactContext?) {
    ...
    if (action == MotionEvent.ACTION_DOWN) {
      ...
      // [com.facebook.react.uimanager.NativeViewHierarchyManager.findTargetTagForTouch] to find react view ID that will be responsible for handling
      // this gesture
      childIsHandlingNativeGesture = false
  ...
  private fun sweepActiveTouchForTag(
      surfaceId: Int, reactTag: Int, reactContext: ReactContext?
  ) {
    if (reactContext == null) {
      return
    }
