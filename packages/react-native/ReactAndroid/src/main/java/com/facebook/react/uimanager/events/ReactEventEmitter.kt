...
internal class ReactEventEmitter(private val reactContext: ReactApplicationContext) :
  ...
  override fun receiveTouches(
    ...
    if (uiManagerType == UIManagerType.DEFAULT) {
      @Suppress("DEPRECATION")
      defaultEventEmitter?.receiveTouches(eventName, touches, changedIndices)
    }
  }
  ...
  override fun receiveTouches(event: TouchEvent) {
    ...
    if (uiManagerType == UIManagerType.FABRIC) {
      ...
      defaultEventEmitter?.let { TouchesHelper.sendTouchesLegacy(it, event) }
    }
  }
   ...
   */
  @Suppress("DEPRECATION")
  private fun getDefaultEventEmitter(): RCTEventEmitter? {
