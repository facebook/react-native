...
import com.facebook.react.internal.featureflags.ReactNativeFeatureFlags.lazyAnimationCallbacks
...
@ReactModule(name = com.facebook.fbreact.specs.NativeAnimatedModuleSpec.NAME)
public class NativeAnimatedModule(reactContext: ReactApplicationContext) :
    com.facebook.fbreact.specs.NativeAnimatedModuleSpec(reactContext),
  ...
  private enum class BatchExecutionOpCodes(val value: Int) {
    ...
    companion object {
      ...
      fun fromId(id: Int): BatchExecutionOpCodes {
        if (valueMap == null) {
          valueMap = entries.toTypedArray()
        }
        // Enum values are 1-indexed, but the value array is 0-indexed
        return Assertions.assertNotNull(valueMap)[id - 1]
      }
    }
  }
  ...
  private inner class ConcurrentOperationQueue {
    ...
    @UiThread
    private fun drainQueueIntoList(maxBatchNumber: Long): List<UIThreadOperation>? {
      if (isEmpty) {
        return null
      }
      ...
      while (true) {
        ...
        val op = peekedOperation
        if (op == null || op.batchNumber > maxBatchNumber) {
          break
        }
        operations.add(op)
  ...
  private val nodesManagerInstance = AtomicReference<NativeAnimatedNodesManager?>()
  ...
  @UIManagerType private var uiManagerType = UIManagerType.DEFAULT
        ...
        object : GuardedFrameCallback(reactContext) {
          override fun doFrameGuarded(frameTimeNanos: Long) {
            try {
              ...
              if (!lazyAnimationCallbacks() || nodesManager.hasActiveAnimations()) {
  ...
  public fun userDrivenScrollEnded(viewTag: Int) {
    ...
    val nodeManager = nodesManagerInstance.get() ?: return
    ...
    val preOperationsUIBlock: UIBlock =
        object : UIBlock {
          override fun execute(nativeViewHierarchyManager: NativeViewHierarchyManager) {
            preOperations.executeBatch(frameNo, nodesManager)
          }
        }
    val operationsUIBlock: UIBlock =
        object : UIBlock {
          override fun execute(nativeViewHierarchyManager: NativeViewHierarchyManager) {
            operations.executeBatch(frameNo, nodesManager)
          }
        }
  ...
  public var nodesManager: NativeAnimatedNodesManager
     ...
     */
    get() {
      if (nodesManagerInstance.get() == null) {
        val reactApplicationContext = getReactApplicationContextIfActiveOrWarn()
        if (reactApplicationContext != null) {
          nodesManagerInstance.compareAndSet(
              null, NativeAnimatedNodesManager(reactApplicationContext))
        }
      }
      return Assertions.assertNotNull(nodesManagerInstance.get())
    }
    set(nodesManager) {
      nodesManagerInstance.set(nodesManager)
    }

  private fun clearFrameCallback() {
    Assertions.assertNotNull(reactChoreographer)
        .removeFrameCallback(
  ...
  private fun initializeLifecycleEventListenersForViewTag(viewTag: Int) {
    uiManagerType = getUIManagerType(viewTag)
    if (uiManagerType == UIManagerType.FABRIC) {
      numFabricAnimations++
    } else {
  ...
  private fun decrementInFlightAnimationsForViewTag(viewTag: Int) {
    @UIManagerType val animationManagerType = getUIManagerType(viewTag)
    ...
    addUnbatchedOperation(
        object : UIThreadOperation() {
          override fun execute(animatedNodesManager: NativeAnimatedNodesManager) {
            var viewTag: Int
            var k = 0
            while (k < opBufferSize) {
              when (BatchExecutionOpCodes.fromId(opsAndArgs.getInt(k++))) {
                BatchExecutionOpCodes.OP_CODE_CREATE_ANIMATED_NODE ->
                    animatedNodesManager.createAnimatedNode(
                        opsAndArgs.getInt(k++), opsAndArgs.getMap(k++))
                BatchExecutionOpCodes.OP_CODE_UPDATE_ANIMATED_NODE_CONFIG ->
                    animatedNodesManager.updateAnimatedNodeConfig(
                        opsAndArgs.getInt(k++), opsAndArgs.getMap(k++))
                BatchExecutionOpCodes.OP_CODE_GET_VALUE ->
                    animatedNodesManager.getValue(opsAndArgs.getInt(k++), null)
                BatchExecutionOpCodes.OP_START_LISTENING_TO_ANIMATED_NODE_VALUE -> {
                  val tag = opsAndArgs.getInt(k++)
                  val listener = AnimatedNodeValueListener { value ->
                    val onAnimatedValueData = Arguments.createMap()
                    onAnimatedValueData.putInt("tag", tag)
                    onAnimatedValueData.putDouble("value", value)
                ...
                BatchExecutionOpCodes.OP_STOP_LISTENING_TO_ANIMATED_NODE_VALUE ->
                    animatedNodesManager.stopListeningToAnimatedNodeValue(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_CONNECT_ANIMATED_NODES ->
                    animatedNodesManager.connectAnimatedNodes(
                        opsAndArgs.getInt(k++), opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_DISCONNECT_ANIMATED_NODES ->
                    animatedNodesManager.disconnectAnimatedNodes(
                        opsAndArgs.getInt(k++), opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_START_ANIMATING_NODE -> {
                  if (lazyAnimationCallbacks()) {
                    enqueueFrameCallback()
                  }
                  animatedNodesManager.startAnimatingNode(
                      opsAndArgs.getInt(k++), opsAndArgs.getInt(k++), opsAndArgs.getMap(k++), null)
                }
                BatchExecutionOpCodes.OP_CODE_STOP_ANIMATION ->
                    animatedNodesManager.stopAnimation(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_SET_ANIMATED_NODE_VALUE ->
                    animatedNodesManager.setAnimatedNodeValue(
                        opsAndArgs.getInt(k++), opsAndArgs.getDouble(k++))
                BatchExecutionOpCodes.OP_CODE_SET_ANIMATED_NODE_OFFSET ->
                    animatedNodesManager.setAnimatedNodeValue(
                        opsAndArgs.getInt(k++), opsAndArgs.getDouble(k++))
                BatchExecutionOpCodes.OP_CODE_FLATTEN_ANIMATED_NODE_OFFSET ->
                    animatedNodesManager.flattenAnimatedNodeOffset(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_EXTRACT_ANIMATED_NODE_OFFSET ->
                    animatedNodesManager.extractAnimatedNodeOffset(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_CONNECT_ANIMATED_NODE_TO_VIEW ->
                    animatedNodesManager.connectAnimatedNodeToView(
                        opsAndArgs.getInt(k++), opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_DISCONNECT_ANIMATED_NODE_FROM_VIEW -> {
                  val animatedNodeTag = opsAndArgs.getInt(k++)
                  viewTag = opsAndArgs.getInt(k++)
                  decrementInFlightAnimationsForViewTag(viewTag)
                  animatedNodesManager.disconnectAnimatedNodeFromView(animatedNodeTag, viewTag)
                }
                BatchExecutionOpCodes.OP_CODE_RESTORE_DEFAULT_VALUES ->
                    animatedNodesManager.restoreDefaultValues(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_DROP_ANIMATED_NODE ->
                    animatedNodesManager.dropAnimatedNode(opsAndArgs.getInt(k++))
                BatchExecutionOpCodes.OP_CODE_ADD_ANIMATED_EVENT_TO_VIEW ->
                    animatedNodesManager.addAnimatedEventToView(
                        opsAndArgs.getInt(k++), opsAndArgs.getString(k++), opsAndArgs.getMap(k++))
                BatchExecutionOpCodes.OP_CODE_REMOVE_ANIMATED_EVENT_FROM_VIEW -> {
                  viewTag = opsAndArgs.getInt(k++)
                  decrementInFlightAnimationsForViewTag(viewTag)
                  animatedNodesManager.removeAnimatedEventFromView(
                      viewTag, opsAndArgs.getString(k++), opsAndArgs.getInt(k++))
                }
                BatchExecutionOpCodes.OP_CODE_ADD_LISTENER,
                BatchExecutionOpCodes.OP_CODE_REMOVE_LISTENERS -> k++
              }
            }
          }
        })
  ...
  public companion object {
    private const val ANIMATED_MODULE_DEBUG = false
    public const val NAME: String = com.facebook.fbreact.specs.NativeAnimatedModuleSpec.NAME
  }
}
