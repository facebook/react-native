...
public class TextAttributeProps private constructor() {
    ...
    protected var fontSizeInput: Float = ReactConstants.UNSET.toFloat()
    protected var lineHeightInput: Float = ReactConstants.UNSET.toFloat()
    protected var letterSpacingInput: Float = Float.NaN
    ...
    protected var heightOfTallestInlineImage: Float = Float.NaN
    ...
    public var letterSpacing: Float
        get() {
            ...
            require(fontSize > 0) {
                "FontSize should be a positive value. Current value: " + fontSize
            }
    ...
    private fun setFontSize(fontSize: Float) {
        ...
        if (fontSize != ReactConstants.UNSET.toFloat()) {
            fontSize =
                if (allowFontScaling)
    ...
    private fun setTextShadowOffset(offsetMap: ReadableMap?) {
        ...
        if (offsetMap != null) {
            if (
                offsetMap.hasKey(PROP_SHADOW_OFFSET_WIDTH) &&
    ...
    public companion object {
            ...
            while (iterator.hasNext()) {
                ...
                when (entry.key) {
                    ...
                    TA_KEY_TEXT_SHADOW_RADIUS ->
                        result.textShadowRadius = entry.doubleValue.toFloat()
                    TA_KEY_TEXT_SHADOW_COLOR -> result.textShadowColor = entry.intValue
                    TA_KEY_TEXT_SHADOW_OFFSET_DX ->
                        result.textShadowOffsetDx = entry.doubleValue.toFloat()
                    TA_KEY_TEXT_SHADOW_OFFSET_DY ->
                        result.textShadowOffsetDy = entry.doubleValue.toFloat()
                    ...
                    TA_KEY_ROLE ->
                        result.setRole(ReactAccessibilityDelegate.Role.entries[entry.intValue])
        ...
        public fun fromReadableMap(props: ReactStylesDiffMap): TextAttributeProps {
            val result = TextAttributeProps()
            result.setNumberOfLines(
                getIntProp(props, ViewProps.NUMBER_OF_LINES, ReactConstants.UNSET)
            )
            result.lineHeight =
                getFloatProp(props, ViewProps.LINE_HEIGHT, ReactConstants.UNSET.toFloat())
            ...
            result.setFontSize(
                getFloatProp(props, ViewProps.FONT_SIZE, ReactConstants.UNSET.toFloat())
            )
            result.color =
                if (props.hasKey(ViewProps.COLOR)) props.getInt(ViewProps.COLOR, 0) else null
            result.color =
                if (props.hasKey(ViewProps.FOREGROUND_COLOR))
        ...
        private fun getBooleanProp(
            ...
            if (props.hasKey(name)) props.getBoolean(name, defaultValue) else defaultValue
        ...
        public fun getLayoutDirection(layoutDirection: String?): Int {
            return when (layoutDirection) {
                null, "undefined" -> ReactConstants.UNSET
