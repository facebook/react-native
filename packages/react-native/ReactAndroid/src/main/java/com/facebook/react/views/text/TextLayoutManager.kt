    ...
    for (i in 0 until fragments.count) {
      ...
      parsedFragments.add(
          FragmentAttributes(
              props,
              fragmentText.length,
              if (fragment.contains(FR_KEY_REACT_TAG)) fragment.getInt(FR_KEY_REACT_TAG)
              else View.NO_ID,
              fragment.contains(FR_KEY_IS_ATTACHMENT) && fragment.getBoolean(FR_KEY_IS_ATTACHMENT),
              if (fragment.contains(FR_KEY_WIDTH)) fragment.getDouble(FR_KEY_WIDTH) else Double.NaN,
              if (fragment.contains(FR_KEY_HEIGHT)) fragment.getDouble(FR_KEY_HEIGHT)
              else Double.NaN))
    }
      ...
      if (fragment.isAttachment) {
        spannable.setSpan(
            TextInlineViewPlaceholderSpan(
                fragment.reactTag, fragment.width.toInt(), fragment.height.toInt()),
            start,
            end,
            spanFlags)
      } else {
