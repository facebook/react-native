...
internal class BorderDrawable(
    ...
    private val borderRadius: BorderRadiusStyle,
    private val borderInsets: BorderInsets,
  ...
  private var borderColorLeft: Int? = null
  private var borderColorRight: Int? = null
  private var borderColorBottom: Int? = null
  private var borderColorTop: Int? = null
  ...
  private val pathForSingleBorder: Path = Path()
  private val pathForOutline: Path = Path()
  ...
  fun getPathForOutline(): Path = pathForOutline

  fun setBorderColor(position: Int, color: Int?) {
    when (position) {
      Spacing.LEFT -> borderColorLeft = color
      Spacing.TOP -> borderColorTop = color
      Spacing.RIGHT -> borderColorRight = color
      Spacing.BOTTOM -> borderColorBottom = color
      Spacing.START -> borderColorLeft = color
      Spacing.END -> borderColorRight = color
      Spacing.ALL -> {
      ...
      Spacing.BLOCK_END -> borderColorBottom = color
      Spacing.BLOCK_START -> borderColorTop = color
      Spacing.BLOCK -> {
        borderColorBottom = color
        borderColorTop = color
      }
  ...
  fun getBorderColor(position: Int): Int {
    return when (position) {
      Spacing.LEFT -> borderColorLeft ?: Color.BLACK
      Spacing.TOP -> borderColorTop ?: Color.BLACK
      Spacing.RIGHT -> borderColorRight ?: Color.BLACK
      Spacing.BOTTOM -> borderColorBottom ?: Color.BLACK
      Spacing.START -> borderColorLeft ?: Color.BLACK
      Spacing.END -> borderColorRight ?: Color.BLACK
      Spacing.ALL ->
          if (borderColorLeft == borderColorRight &&
              borderColorLeft == borderColorBottom &&
              borderColorLeft == borderColorTop)
              borderColorLeft ?: Color.BLACK
          else Color.BLACK
      Spacing.BLOCK_END -> borderColorBottom ?: Color.BLACK
      Spacing.BLOCK_START -> borderColorTop ?: Color.BLACK
      Spacing.BLOCK ->
          if (borderColorBottom == borderColorTop) borderColorTop ?: Color.BLACK else Color.BLACK
      Spacing.HORIZONTAL -> Color.BLACK
      Spacing.VERTICAL -> Color.BLACK
  ...
  private fun drawRectangularBorders(canvas: Canvas) {
    ...
    if (borderLeft > 0 || borderRight > 0 || borderTop > 0 || borderBottom > 0) {
      ...
      if (instance.doLeftAndRightSwapInRTL(context)) {
      ...
      if (fastBorderColor != 0) {
        if (Color.alpha(fastBorderColor) != 0) {
          ...
          borderPaint.color = fastBorderColor
          ...
          if (borderLeft > 0) {
            ...
            canvas.drawPath(pathForSingleBorder, borderPaint)
          }
          if (borderTop > 0) {
            pathForSingleBorder.reset()
  ...
  private fun drawRoundedBorders(canvas: Canvas) {
    ...
    if (borderWidth.top > 0 ||
      ...
      // In the case of uneven border widths/colors draw quadrilateral in each direction
        ...
        val isRTL = layoutDirection == View.LAYOUT_DIRECTION_RTL
        var colorStart: Int = getBorderColor(Spacing.START)
        var colorEnd: Int = getBorderColor(Spacing.END)
        if (instance.doLeftAndRightSwapInRTL(context)) {
          if (!isBorderColorDefined(Spacing.START)) {
            colorStart = colorLeft
          }
          ...
          val directionAwareColorLeft = if (isRTL) colorEnd else colorStart
          val directionAwareColorRight = if (isRTL) colorStart else colorEnd
          val isColorStartDefined: Boolean = isBorderColorDefined(Spacing.START)
          val isColorEndDefined: Boolean = isBorderColorDefined(Spacing.END)
  ...
  private fun isBorderColorDefined(position: Int): Boolean {
    return when (position) {
      Spacing.LEFT -> borderColorLeft != null
      Spacing.TOP -> borderColorTop != null
      Spacing.RIGHT -> borderColorRight != null
      Spacing.BOTTOM -> borderColorBottom != null
      Spacing.START -> borderColorLeft != null
      Spacing.END -> borderColorRight != null
      Spacing.ALL ->
          if (borderColorLeft == borderColorRight &&
              borderColorLeft == borderColorBottom &&
              borderColorLeft == borderColorTop)
      ...
      Spacing.BLOCK_END -> borderColorBottom != null
      Spacing.BLOCK_START -> borderColorTop != null
      Spacing.BLOCK -> if (borderColorBottom == borderColorTop) borderColorTop != null else false
  ...
  private fun updatePathEffect(borderWidth: Int) {
    ...
    borderStyle?.let { pathEffectForBorderStyle = getPathEffect(it, borderWidth.toFloat()) }
    borderPaint.setPathEffect(pathEffectForBorderStyle)
  }
  ...
  private fun updatePath() {
    ...
    if (Color.alpha(colorLeft) != 0 &&
      ...
      innerClipTempRectForBorderRadius?.top =
          innerClipTempRectForBorderRadius?.top?.plus(borderWidth.top)
      innerClipTempRectForBorderRadius?.bottom =
          innerClipTempRectForBorderRadius?.bottom?.minus(borderWidth.bottom)
      innerClipTempRectForBorderRadius?.left =
          innerClipTempRectForBorderRadius?.left?.plus(borderWidth.left)
      innerClipTempRectForBorderRadius?.right =
          innerClipTempRectForBorderRadius?.right?.minus(borderWidth.right)
    }

    tempRectForCenterDrawPath?.top = tempRectForCenterDrawPath?.top?.plus(borderWidth.top * 0.5f)
    tempRectForCenterDrawPath?.bottom =
        tempRectForCenterDrawPath?.bottom?.minus(borderWidth.bottom * 0.5f)
    tempRectForCenterDrawPath?.left = tempRectForCenterDrawPath?.left?.plus(borderWidth.left * 0.5f)
    tempRectForCenterDrawPath?.right =
        tempRectForCenterDrawPath?.right?.minus(borderWidth.right * 0.5f)
