...
public abstract class Event<T : Event<T>?> {
   ...
   */
  @JvmOverloads
  protected fun init(surfaceId: Int, viewTag: Int, timestampMs: Long = uptimeMillis()) {
   ...
   * coalescing key.
   */
  public fun coalesce(otherEvent: Event<T>?): Event<T>? =
   ...
   */
  internal fun getEventDataInternal(): WritableMap? = getEventData()
   ...
   * sublcasses, but inside the package.
   */
  internal fun getEventCategoryInternal(): Int = getEventCategory()
   ...
   * sublcasses, but inside the package.
   */
  internal fun experimental_isSynchronousInternal(): Boolean = experimental_isSynchronous()
