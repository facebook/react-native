...
public class JavaOnlyMap() : ReadableMap, WritableMap {
    ...
    public fun deepClone(map: ReadableMap?): JavaOnlyMap {
      ...
      while (iter.hasNextKey()) {
        ...
        when (type) {
          ...
          ReadableType.Map -> res.putMap(propKey, deepClone(checkNotNull(map.getMap(propKey))))
          ReadableType.Array ->
  ...
  override fun getLong(name: String): Long = (checkNotNull(backingMap[name] as Number).toLong())
