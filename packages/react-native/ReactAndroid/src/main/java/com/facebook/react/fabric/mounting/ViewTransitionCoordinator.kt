...
internal class ViewTransitionCoordinator(private val surfaceId: Int) {
  ...
  fun markViewInTransition(
    ...
    if (transitioning) {
      if (viewsInTransition.contains(tag)) {
        Log.d("HannoDebug", "View [$tag] is already marked as in transition on surface [$surfaceId], ignoring")
        return
      }

      viewsInTransition.add(tag)
      Log.d("HannoDebug", "Marking view [$tag] as in transition on surface [$surfaceId]")
      ...
      if (view != null) {
        ...
        val listener = object : View.OnAttachStateChangeListener {
          ...
          override fun onViewDetachedFromWindow(v: View) {
            Log.d("HannoDebug", "[$tag] detached from window, running detach queue")
            ...
            UiThreadUtil.runOnUiThread {
              viewsInTransition.remove(tag) // TODO: i think this should be the devs responsibility
              Log.d("HannoDebug", "[$tag] detached, run queue now!")
              onDetach.run()
            }
          }
        }
      ...
      Log.d("HannoDebug", "Marking view [$tag] as NOT in transition on surface [$surfaceId]")
  ...
  fun shouldEnqueueOperation(childTag: Int, parentTag: Int, checkTransitionStatus: Boolean = true): Boolean {
    ...
    if (parentQueues.containsKey(parentTag)) {
      Log.d("HannoDebug", "shouldEnqueueOperation $parentTag:$childTag -> true (parent has queue)")
      return true
    }
    ...
    if (checkTransitionStatus && viewsInTransition.contains(childTag)) {
      Log.d(
        "HannoDebug",
        "shouldEnqueueOperation $parentTag:$childTag -> true (child in transition)"
      )
      return true
    }
  ...
  fun enqueueOperation(operation: ViewOperation) {
    ...
    if (ReactBuildConfig.DEBUG) {
      Log.d(
        TAG,
        "[$surfaceId] Enqueued operation: $operation"
      )
    }
  }
  ...
  fun drainOperationsForChild(childTag: Int, manager: SurfaceMountingManager) {
    ...
    do {
      ...
      if (parentOrderForChild.isNullOrEmpty()) {
        Log.d("HannoDebug", "No pending operations for child tag [$childTag]. Nothing to drain.")
        break
      }
      ...
      if (drainingParentTag == parentTag) {
        ...
        Log.d("HannoDebug", "Already draining parentTag=$parentTag, breaking to avoid double loop")
        break
      }
      drainingParentTag = parentTag

      Log.d("HannoDebug", "Draining operations $parentTag:$childTag, queue=$queue")
      madeProgress = drainQueue(parentTag, queue, manager)
      Log.d(
        "HannoDebug",
        "Drained for $parentTag:$childTag, madeProgress=$madeProgress, queue=$queue, childToParentOrder=$childToParentOrder"
      )
  ...
  private fun drainQueue(
    ...
    while (iterator.hasNext()) {
      ...
      if (!operation.isReadyToExecute(this) || viewsInTransition.contains(operation.childTag)) {
        Log.d("HannoDebug", "Operation not ready to execute: $operation")
        break
      }
      ...
      Log.d("HannoDebug", "Executed operation: $operation, updated childToParentOrder=$childToParentOrder, queue=$queue")
      madeProgress = true
    }
