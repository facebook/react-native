 ...
 *
 * Lifecycle events via a [LifecycleEventListener] will still always happen on the UI
 * thread.
 */
@DoNotStrip
public class ModuleHolder {
  ...
  public constructor(nativeModule: NativeModule) {
    ...
    PrinterHolder.printer
        .logMessage(ReactDebugOverlayTags.NATIVE_MODULE, "NativeModule init: %s", name)
  }
  ...
  @Synchronized
  public fun hasInstance(): Boolean = internalModule != null
  ...
  public val module: NativeModule
    get() {
      ...
      synchronized(this) {
        ...
        if (safeModule != null) {
          return safeModule
          // if `internalModule` has not been set, and no one is creating it. Then this thread should call
          // create
        } else if (!isCreating) {
          shouldCreate = true
          isCreating = true
      ...
      if (shouldCreate) {
        module = create()
        // Once module is built (and initialized if markInitializable has been called), modify `internalModule`
