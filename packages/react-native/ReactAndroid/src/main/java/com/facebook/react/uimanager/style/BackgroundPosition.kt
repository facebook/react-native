...
public class BackgroundPosition(
  ...
  public companion object {
    public fun parse(backgroundPositionMap: ReadableMap?): BackgroundPosition? {
      ...
      val top =
        if (backgroundPositionMap.hasKey("top") && backgroundPositionMap.getType("top") != ReadableType.Null) {
          LengthPercentage.setFromDynamic(backgroundPositionMap.getDynamic("top"), true)
        } else null

      val left =
        if (backgroundPositionMap.hasKey("left") && backgroundPositionMap.getType("left") != ReadableType.Null) {
          LengthPercentage.setFromDynamic(backgroundPositionMap.getDynamic("left"), true)
        } else null

      val right =
        if (backgroundPositionMap.hasKey("right") && backgroundPositionMap.getType("right") != ReadableType.Null) {
