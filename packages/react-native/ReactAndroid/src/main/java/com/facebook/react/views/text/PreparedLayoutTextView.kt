...
internal class PreparedLayoutTextView(context: Context) : ViewGroup(context), ReactCompoundView {
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    val textLayout = preparedLayout?.layout
    if (textLayout != null) {
      if (selection != null) {
  ...
  public fun setSelection(start: Int, end: Int) {
    val textLayout = checkNotNull(preparedLayout)
    if (start < 0 || end > textLayout.layout.text.length || start >= end) {
      throw IllegalArgumentException(
          "setSelection start and end are not in valid range. start: $start, end: $end, text length: ${textLayout.layout.text.length}")
    }

    val textSelection = selection
    if (textSelection == null) {
      val selectionPath = Path()
      textLayout.layout.getSelectionPath(start, end, selectionPath)
      selection = TextSelection(start, end, selectionPath)
    } else {
      textSelection.start = start
      textSelection.end = end
      textLayout.layout.getSelectionPath(start, end, textSelection.path)
    }
  ...
  override fun onTouchEvent(event: MotionEvent): Boolean {
    ...
    if (action == MotionEvent.ACTION_UP) {
      ...
      val textLayout = checkNotNull(preparedLayout)
      val start = (textLayout.layout.text as Spanned).getSpanStart(clickableSpan)
      val end = (textLayout.layout.text as Spanned).getSpanEnd(clickableSpan)
  ...
  private fun getTextOffsetAt(x: Int, y: Int): Int {
    val textLayout = preparedLayout ?: return -1
    val line = textLayout.layout.getLineForVertical(y)

    val left: Float
    val right: Float

    if (textLayout.layout.alignment == Layout.Alignment.ALIGN_CENTER) {
      /**
       * [Layout#getLineLeft] and [Layout#getLineRight] properly account for paragraph margins on
       * centered text.
       */
      left = textLayout.layout.getLineLeft(line)
      right = textLayout.layout.getLineRight(line)
    } else {
      /**
       ...
       * rest from there.
       */
      val rtl = textLayout.layout.getParagraphDirection(line) == Layout.DIR_RIGHT_TO_LEFT
      left =
          if (rtl) (textLayout.layout.width - textLayout.layout.getLineMax(line))
          else textLayout.layout.getParagraphLeft(line).toFloat()
      right =
          if (rtl) textLayout.layout.getParagraphRight(line).toFloat()
          else textLayout.layout.getLineMax(line)
    }
    ...
    return try {
      textLayout.layout.getOffsetForHorizontal(line, x.toFloat())
