  ...
  public fun findNextClippedElement(
    ...
    val nextFocusableViewMetrics =
        (uimanager as FabricUIManager).findNextFocusableElementMetrics(
            scrollView.getChildAt(0).id, focused.id, direction)

    if (nextFocusableViewMetrics != null && scrollView is ReactClippingViewGroup) {

      scrollView.updateClippingRectForNavigation(nextFocusableViewMetrics.parentId)
