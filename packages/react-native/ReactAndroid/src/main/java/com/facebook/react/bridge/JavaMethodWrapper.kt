...
internal class JavaMethodWrapper(
  ...
  private fun buildArgumentExtractors(paramTypes: Array<Class<*>>): Array<ArgumentExtractor<*>> {
    ...
    while (i < paramTypes.size) {
      ...
      val extractor: ArgumentExtractor<*> = when (argumentClass) {
        ...
        else -> throw RuntimeException("Got unknown argument class: ${argumentClass.simpleName}")
      }
  ...
  override fun invoke(jsInstance: JSInstance, parameters: ReadableArray) {
    ...
    try {
      ...
      val validatedArguments = requireNotNull(arguments) {
        "processArguments failed: 'arguments' is null."
      }
      val validatedArgumentExtractors = requireNotNull(argumentExtractors) {
  ...
  companion object {
    ...
    private val ARGUMENT_EXTRACTOR_PROMISE: ArgumentExtractor<Promise> =
      object : ArgumentExtractor<Promise>() {
        ...
        override fun extractArgument(
          ...
          val resolve =
            ARGUMENT_EXTRACTOR_CALLBACK.extractArgument(
                jsInstance,
                jsArguments,
                atIndex
            )
    ...
    private val DEBUG = PrinterHolder.printer.shouldDisplayLogMessage(ReactDebugOverlayTags.BRIDGE_CALLS)
     ...
     */
    private fun createInvokeExceptionMessage(traceName: String): String = "Could not invoke $traceName"
  }
}
