...
import type {NativeModeChangeEvent} from './unstable_VirtualViewNativeComponent';

import StyleSheet from '../../../../Libraries/StyleSheet/StyleSheet';
import * as ReactNativeFeatureFlags from '../../../../src/fb_internal/ReactNativeFeatureFlags';
import VirtualViewNativeComponent from './unstable_VirtualViewNativeComponent';
...
type VirtualViewComponent = component(
  children?: React.Node,
  nativeID?: string,
  // TODO (T226306020): Delete this after D75714323 ships to production.
  ...
  component VirtualView(
    ...
    const handleModeChange = (
      ...
      if (ReactNativeFeatureFlags.enableVirtualViewDebugFeatures()) {
        performance.mark(
          `Track:Fling:VirtualView onModeChange | nativeID=${nativeID ?? ''} mode={${VirtualViewMode.getName(mode)}`,
        );
      }
    ...
    if (ReactNativeFeatureFlags.enableVirtualViewDebugFeatures()) {
      performance.mark(
        `Track:Fling:VirtualView render | nativeID=${nativeID ?? ''} isHidden={${String(isHidden)}`,
      );
    }
