...
- (void)setText:(NSString *)text
{
  ...
  if (eventLag == 0 && ![text isEqualToString:self.text]) {
    ...
    if (selection.empty) {
      ...
      long offsetStart = [self offsetFromPosition:self.beginningOfDocument toPosition:selection.start];
      long offsetFromEnd = oldText.length - offsetStart;
      long newOffset = text.length - offsetFromEnd;
