...
Now install that version of `react-native` in your project with `npm install --save`. For example, to upgrade to the version `0.21`, in a terminal run:

```sh
$ npm install --save react-native@0.21
```
